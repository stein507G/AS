...
{
	char cModifyChar;	// ex) "IPADDR"

	char cSerialNO[17];		// ex)
	
	char cInstallDate[4];			// ex)	20/21/09/01
	char cCurDate[4];			// ex)	20/21/09/01
	char cCurTime[4];			// ex)	--/14/30/35
	
	char cFirmwareVersion[3];			// 01/01/03  버전		 ex) "1.1.3" 
	char cMachineVersion[3];			// 01/03/08  버전		 ex) "1.3.8"

	char cIPAddress[4];	// ex) "10.10.10.52"
	char cGateway[4];	// ex) "10.10.10.1"
	char cNetmask[4];			// ex) "255.255.255.0"
	short int PortNo;		// ex) "4242"
	char cMacAddress[6];	// ex) "00:11:22:33:44:55"	

	float fCalTemp[2];
	char TempConModule;		// 온도제어 모듈 유무 
