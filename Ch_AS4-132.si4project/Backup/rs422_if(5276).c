...
iprintf("RS422_CMD_PACKET_SEND Packet.nPacketCode=%x len=%d\n",pPacket->nPacketCode,rxlen);
...
iprintf("nPacketLength=%d\n",pPacket->nPacketLength);
					...
					ret = r_wite_TxMem(pPacket->nPacketLength, &tx);
...
//	MSS_MAC_disable_intr();
	...
	if (bf->Tail4 + size4 >= buf_size4) {
		int copySize = ((buf_size4 - bf->Tail4) << 2);
		memcpy(&bf->iData[bf->Tail4], &pBuffer[0], copySize);
		memcpy(bf->iData, &pBuffer[copySize], size - copySize);
	}
	else {
		memcpy(&bf->iData[bf->Tail4], &pBuffer[0], size);
	}

	bf->Tail4 = (bf->Tail4 + size4) % buf_size4;

	ret = bf->Head4 <= bf->Tail4 ? bf->Tail4 - bf->Head4 : buf_size4 + bf->Tail4 - bf->Head4;
...
//	---		memcpy((uint8_t *)(pTxMem+1), (const void*)&bf->Data[bf->Head], copy);
		size4 = copy >> 2;
		pData = (uint32_t *)(&bf->Data[bf->Head]);
...
//		return 0;
	...
	if (bf->Head4 + size4 > buf_size4) {
		int copy = ((buf_size4 - bf->Head4) << 2);

		memcpy(&pBuffer[0], (const void*)&bf->iData[bf->Head4], copy);
		memcpy(&pBuffer[copy], (const void*)bf->iData, size - copy);
	}
	else {
