...
#endif
#if 0
	unsigned char saveFlag_2nd[6];	// x,y,z,syr,val,inj
	signed char	value_2nd[6]; // 홈방향으로 이동하면(+) / 홈과 반대로 이동(-)
	
	unsigned char saveVL_2nd[3];		// 바이알 좌표 조정값 저장여부 0:40vial ,1:60vial ,2:96vial - wellplate
	signed char leftVial_x_2nd[3];
	signed char leftVial_y_2nd[3];
	
	unsigned char saveVR_2nd[3];		// 바이알 좌표 조정값 저장여부 0:40vial ,1:60vial ,2:96vial - wellplate	
	signed char rightVial_x_2nd[3];
	signed char rightVial_y_2nd[3];

	unsigned char saveWash_2nd;		// wash port , waste port 좌표 조정값 저장여부
	signed char wash_x_2nd;
	signed char wash_y_2nd;

	unsigned char saveWaste_2nd;		// wash port , waste port 좌표 조정값 저장여부	
	signed char waste_x_2nd;
	signed char waste_y_2nd;

//#if VIAL_HEIGHT_DATUM	// Board  Rev 3 // 기구물 변경 6
	unsigned char saveVialHeight_2nd;		// 
	signed char vialHeight_2nd;	

	unsigned char saveVialHeight_xy_2nd;
	signed char vialHeight_x_2nd;		
	signed char vialHeight_y_2nd;			
//#endif

#if MICRO_PUMP_VOLTAGE_ADJUSTMENT
	unsigned char saveMicroPump_2nd;
	unsigned char microPumpVolt_2nd;
#endif	

#if	EEPROM_ADJUST_DATA_SAVE_FIX
//	uint32_t dummy_2nd[4];
#endif
#endif	
} ADJUST_DEV_DATA_t;
