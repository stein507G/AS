...
{
//pos  : pVial->uPos.etc
//tray :  pVial->uPos.tray
int32_t pos;// = pVial->uPos.x; //pVial->uPos.etc;
//int32_t y_pos = pVial->uPos.y; //pVial->uPos.etc;
uint32_t Tray = pVial->uPos.tray;
...
int32_t desStep;	
	...
	CMD_ENCODER_CLR_CNT_ENABLE(MOTOR_X);
	devState.mActSet[MOTOR_X].pos = pos;	// vial 좌표 a~e (a1 .. e12)
	devState.mActSet[MOTOR_X].Tray = Tray;
	devState.mActSet[MOTOR_X].option = option;

	cur = devState.curStep[MOTOR_X];
//desStep = devState.curStep[MOTOR_X];
...
//					act = xyzCoord.vialStep_L_X[pos] + calX * y_pos / Ycount;
					pos = pVial->uPos.x;
					act = xyzCoord.vialStep_L_X[pos] + devSet.adjust.leftVial_x[sysConfig.trayLeft] * pVial->uPos.y / devSet.trayLeft_colNo;
...
//desStep = act;
				...
				case TRAY_RIGHT:
					pos = pVial->uPos.x;
					act = xyzCoord.vialStep_R_X[pos] + devSet.adjust.rightVial_x[sysConfig.trayRight] * pVial->uPos.y / devSet.trayRight_colNo;
					devState.curStep[MOTOR_X] = act;
//desStep = act;					
					devState.curTray = Tray;
					break;
	...
	switch(option) {
		case ACT_XY_POS:
			switch(Tray) {
				case TRAY_LEFT:
pos = pVial->uPos.y;
//act = xyzCoord.vialStep_L_X[pos] + devSet.adjust.leftVial_x[sysConfig.trayLeft] * pVial->uPos.x / devSet.trayLeft_colNo;			
					act = xyzCoord.vialStep_L_Y[pos];
