...
#else
volatile uint32_t * pTxMem = (uint32_t  *)(0x50008000);	// 0x2000 (32bit) -- 0010 0000 0000 0000 << 2  --> 1000 0000 0000 0000 (0x8000) --> 0x50008000
volatile uint32_t * pRxMem = (uint32_t  *)(0x50009000);	// 0x2400 (32bit) -- 0010 0100 0000 0000 << 2  --> 1001 0000 0000 0000 (0x9000) --> 0x50009000

volatile uint8_t * pTxMem_byte = (uint8_t  *)(0x50008000);	// 0x2000 (32bit) -- 0010 0000 0000 0000 << 2  --> 1000 0000 0000 0000 (0x8000) --> 0x50008000
volatile uint8_t * pRxMem_byte = (uint8_t  *)(0x50009000);	// 0x2400 (32bit) -- 0010 0100 0000 0000 << 2  --> 1001 0000 0000 0000 (0x9000) --> 0x50009000
#endif
...
//		memcpy(&pTxMem[1], (const void*)&bf->Data[bf->Head], copy);
		memcpy((uint8_t *)(pTxMem_byte+copy), (const void*)bf->Data, size - copy);
...
//memcpy((&pTxMem[1]), &pBuffer[0], size);

//		memcpy((&pTxMem[1]), &LcasState, size);
		memcpy((&pTxMem_byte[4]), &LcasState, size);
...
iprintf("[%d]%x %x\n",i,pBuffer[i], pTxMem_byte[i]);
	...
	iprintf("r_save_TxMem H=%d[%d] T=%d[%d] len=%d[%d][%d] nPacketCode=%x addr=%x\n",tx.Head ,bf->Head,tx.Tail, bf->Tail, pBuffer[0], bf->Data[bf->Head], size,pTxMem[2],pTxMem+1);
