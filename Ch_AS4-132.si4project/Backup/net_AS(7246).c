...
static err_t net_AS_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
  ...
  int len;
	...
	char acData_in[PACKET_BUFF_SIZE];// = {0};
//	char acData_in[512]; // [PACKET_BUFF_SIZE];
	char *pData_in;
...
// ryu debug
// dp("A-tcp_recved tot_len=%d\n",p->tot_len);
#if 0
len = p-> tot_len;  
//pData_in = q->payload;
pData_in = p->payload;
for(i=0;i<len;i++) {
	dp("[%3d] = 0x%02x addr[0x%08x]\n",i,pData_in[i],pData_in);		
}
#endif
    /* Check the name if NULL, no data passed, return withh illegal argument error */
		if(!name) 
	  {
		...
		memcpy(acData_in + iSaveDatainPos, pData_in, len);
		iSaveDatainPos += len;
...
//dp("iSaveDatainPos=%d\n",iSaveDatainPos);
	...
	//if(iPacketLength > 256) iPacketLength = 256;
...
//    dp("A-pbuf_free\n");
  ...
  {
dfp("A-tcp_close signal\n");  
// 20200109  
#if FIX_TCP_CLOSE_EXCUTE_2  
#else
    /* When the pbuf is NULL and the err is ERR_OK, the remote end is closing the connection. */
    /* We free the allocated memory and we close the connection */
    mem_free(name);
		
dfp("A-tcp_close excute \n");
...
//    tcp_close(pcb);
  ...
  tcp_err(pcb, net_AS_conn_err);
  tcp_recv(pcb, net_AS_recv);
...
void net_AS_init(void)
{
  ...
  tcp_bind(pcb, IP_ADDR_ANY, AS_MY_PORT);

  /* Set the connection to the LISTEN state */
  pcb = tcp_listen(pcb);				

  /* Specify the function to be called when a connection is established */	
  tcp_accept(pcb, net_AS_accept);
}
...
#else
SEND_LCAS_PACKET(PACKCODE_LCAS_SYSTEM_INFORM, sysInform, sysInform);
#endif
}	
...
//	if(strcmp(tmp.cModel, sysInform.cModel)) {			// 같지 않으면 
...
#if 0
		if(sizeof(tmp) == sizeof(sysInform) ) {
			memcpy(&sysInform.cSysDate, &tmp.cSysDate, sizeof(tmp.cSysDate));
			memcpy(&sysInform.cSysTime, &tmp.cSysTime, sizeof(tmp.cSysTime));

//			set_RtcTime(sysInform.cSysDate.year,sysInform.cSysDate.month,sysInform.cSysDate.date,
//				sysInform.cSysDate.hour,sysInform.cSysDate.minute,sysInform.cSysDate.second);		
			
dfp("set_RtcTime : net_CheckModel!!!\n %d.%d.%d  %d:%d:%d \n",
...
//void PacketParse(unsigned int nPacketCode, char *pData_in, LPVOID lpArg)
//void PacketParse(unsigned int nPacketCode, char *pData)//, void *lpArg)
int PacketParse(unsigned int nPacketCode, char *pData)
{
	int i;
...
//dfp("curInjCnt = %d sample.pos = [%d,%d] injectVial.pos = [%d,%d]\n"
//	,LcasState.curInjCnt,LcasState.sample.uPos.x,LcasState.sample.uPos.y,LcasState.injectVial.uPos.x,LcasState.injectVial.uPos.y);
