...
#if RS422_MY_ID	== RS422_ID_AS
	// selfMsgBufHeader[24]
	((LCAS_PACKET *)selfMsgBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(LCAS_SELFMSG_t);
	((LCAS_PACKET *)selfMsgBufHeader)->nDeviceID = (PACKET_DEVICE_ID_YLAS);
	((LCAS_PACKET *)selfMsgBufHeader)->nPacketCode = (PACKCODE_LCAS_SLFEMSG);
	((LCAS_PACKET *)selfMsgBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)selfMsgBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((LCAS_PACKET *)selfMsgBufHeader)->nSlotSize = sizeof(LCAS_SELFMSG_t);

	// stateBufHeader[24]
	((LCAS_PACKET *)stateBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(LCAS_STATE_t);
	((LCAS_PACKET *)stateBufHeader)->nDeviceID = (PACKET_DEVICE_ID_YLAS);
	((LCAS_PACKET *)stateBufHeader)->nPacketCode = (PACKCODE_LCAS_SLFEMSG);
	((LCAS_PACKET *)stateBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)stateBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((LCAS_PACKET *)stateBufHeader)->nSlotSize = sizeof(LCAS_STATE_t);

	// diagDataBufHeader[24]
	((LCAS_PACKET *)diagDataBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(LCAS_DIAG_DATA_t);
	((LCAS_PACKET *)diagDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_YLAS);
	((LCAS_PACKET *)diagDataBufHeader)->nPacketCode = (PACKCODE_LCAS_SLFEMSG);
	((LCAS_PACKET *)diagDataBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)diagDataBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((LCAS_PACKET *)diagDataBufHeader)->nSlotSize = sizeof(LCAS_DIAG_DATA_t);

#elif RS422_MY_ID	== RS422_ID_OVEN
	// selfMsgBufHeader[24]
	((LCAS_PACKET *)selfMsgBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(YL9030_OVEN_SELFMSG);
	((LCAS_PACKET *)selfMsgBufHeader)->nDeviceID = (PACKET_DEVICE_ID_YLAS);
	((LCAS_PACKET *)selfMsgBufHeader)->nPacketCode = (PACKCODE_YL9030_SELFMSG);
	((LCAS_PACKET *)selfMsgBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)selfMsgBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((LCAS_PACKET *)selfMsgBufHeader)->nSlotSize = sizeof(YL9030_OVEN_SELFMSG);

	// stateBufHeader[24]
	((LCAS_PACKET *)stateBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(YL9030_OVEN_STATE);
	((LCAS_PACKET *)stateBufHeader)->nDeviceID = (PACKET_DEVICE_ID_YLAS);
	((LCAS_PACKET *)stateBufHeader)->nPacketCode = (PACKCODE_YL9030_STATE);
	((LCAS_PACKET *)stateBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)stateBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((LCAS_PACKET *)stateBufHeader)->nSlotSize = sizeof(YL9030_OVEN_STATE);

	// diagDataBufHeader[24]
	((LCAS_PACKET *)diagDataBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(YL9030_OVEN_DIAGDATA);
	((LCAS_PACKET *)diagDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_YLAS);
	((LCAS_PACKET *)diagDataBufHeader)->nPacketCode = (PACKCODE_YL9030_DIAGDATA);
	((LCAS_PACKET *)diagDataBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)diagDataBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((LCAS_PACKET *)diagDataBufHeader)->nSlotSize = sizeof(YL9030_OVEN_DIAGDATA);

#elif RS422_MY_ID == RS422_ID_UVD
	// selfMsgBufHeader[24]
	((YL9000HPLC_PACKET *)selfMsgBufHeader)->nPacketLength = sizeof(YL9000HPLC_PACKET) + sizeof(YL9020_UVD_SELFMSG);
	((YL9000HPLC_PACKET *)selfMsgBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((YL9000HPLC_PACKET *)selfMsgBufHeader)->nPacketCode = (PACKCODE_YL9020_SELFMSG);
	((YL9000HPLC_PACKET *)selfMsgBufHeader)->nEventIndex = 0;
	((YL9000HPLC_PACKET *)selfMsgBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((YL9000HPLC_PACKET *)selfMsgBufHeader)->nSlotSize = sizeof(YL9020_UVD_SELFMSG);

	// stateBufHeader[24]
	((YL9000HPLC_PACKET *)stateBufHeader)->nPacketLength = sizeof(YL9000HPLC_PACKET) + sizeof(LCAS_STATE_t);
	((YL9000HPLC_PACKET *)stateBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((YL9000HPLC_PACKET *)stateBufHeader)->nPacketCode = (PACKCODE_YL9020_STATE);
	((YL9000HPLC_PACKET *)stateBufHeader)->nEventIndex = 0;
	((YL9000HPLC_PACKET *)stateBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((YL9000HPLC_PACKET *)stateBufHeader)->nSlotSize = sizeof(LCAS_STATE_t);

	// diagDataBufHeader[24]
	((YL9000HPLC_PACKET *)diagDataBufHeader)->nPacketLength = sizeof(YL9000HPLC_PACKET) + sizeof(LCAS_DIAG_DATA_t);
	((YL9000HPLC_PACKET *)diagDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((YL9000HPLC_PACKET *)diagDataBufHeader)->nPacketCode = (PACKCODE_YL9020_DIAGDATA);
	((YL9000HPLC_PACKET *)diagDataBufHeader)->nEventIndex = 0;
	((YL9000HPLC_PACKET *)diagDataBufHeader)->nSlotOffset = 0; // ((int)&selfMsg) - ((int)&selfMsg);
	((YL9000HPLC_PACKET *)diagDataBufHeader)->nSlotSize = sizeof(LCAS_DIAG_DATA_t);

	// scanDataBufHeader[24]
	((YL9000HPLC_PACKET *)scanDataBufHeader)->nPacketLength = sizeof(YL9000HPLC_PACKET) + sizeof(YL9020_UVD_SCANDATA);
	((YL9000HPLC_PACKET *)scanDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((YL9000HPLC_PACKET *)scanDataBufHeader)->nPacketCode = (PACKCODE_YL9020_SCANDATA);
	((YL9000HPLC_PACKET *)scanDataBufHeader)->nEventIndex = 0;
	((YL9000HPLC_PACKET *)scanDataBufHeader)->nSlotOffset = 0;
	((YL9000HPLC_PACKET *)scanDataBufHeader)->nSlotSize = sizeof(YL9020_UVD_SCANDATA);

	// acqDataBufHeader[24]
	((YL9000HPLC_PACKET *)acqDataBufHeader)->nPacketLength = sizeof(YL9000HPLC_PACKET) + sizeof(YL9020_UVD_ACQDATA);
	((YL9000HPLC_PACKET *)acqDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((YL9000HPLC_PACKET *)acqDataBufHeader)->nPacketCode = (PACKCODE_YL9020_ACQDATA);
	((YL9000HPLC_PACKET *)acqDataBufHeader)->nEventIndex = 0;
	((YL9000HPLC_PACKET *)acqDataBufHeader)->nSlotOffset = 0;
	((YL9000HPLC_PACKET *)acqDataBufHeader)->nSlotSize = sizeof(YL9020_UVD_ACQDATA);
#endif
...
/*
				switch(pPacket->nPacketCode) {
					case 
//				packetCode = FPGA_READ_WORD(RS422_RX_PACKET_CODE_ADDR);
				PacketParse(pPacket->nPacketCode, acData_in) == RE_ERR) {
*/
	...
	switch(nPacketCode)
	{
		case PACKCODE_LCAS_SYSTEM_INFORM:
			if(IS_REQUEST_PACKET(pData)) {	// 데이터 요청
				...
				SEND_RESPONSE_LCAS_PACKET(sysInform,pData);
				RS422_BUFF_LCAS_PACKET(sysInform,pData);
		...
		if(tx.Head == tx.Tail) {
			...
			if(TSTAT&0x80) {
				...
				if (data_cnt >= PACKET_HEADER_SIZE) {
					ret = r_peek(pBuffer,  PACKET_SIZE_MIN, &tx);
...
//if(rs422if.TSTAT == 0)
	...
	if(rs422if.selfMsgBufHead != rs422if.selfMsgBufTail) {
		...
		if(TSTAT&0x80) {
			...
			pTxMem[0] = STATE_REQUEST_CMD;			// FPGA에서 자동으로 결정된다. (0xB0)
			memcpy((uint8_t *)(pTxMem+1), selfMsgBufHeader, sizeof(selfMsgBufHeader));		// Packet Header
			memcpy((uint8_t *)(pTxMem+7), &rs422_selfMsgBuf[rs422if.selfMsgBufTail], sizeof(LCAS_SELFMSG_t));
			FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(LCAS_SELFMSG_t) >> 2) + 2); 	//packetHeader(6)  tx_len : uint8_t --> uint32_t ,	Head(1) , Tail(1)
			FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
			rs422if.selfMsgBufTail = (++rs422if.selfMsgBufTail) & 0x7;
			rs422if.TSTAT = 1;
		}
		else {
			rs422if.TSTAT = 0;
...
//			int length = tx.Data[tx.Head];
		
			if((tx.Head + PACKET_HEADER_SIZE) <= RS422_TX_DATA_BUF_SIZE) { //tx.Tail) {	
				memcpy(&Header, &tx.Data[tx.Head], PACKET_HEADER_SIZE);		// Packet Header
				Header.nPacketLength;
				if((tx.Head + Header.nPacketLength) <= RS422_TX_DATA_BUF_SIZE) { //tx.Tail) {	
				
				memcpy((uint8_t *)(pTxMem+1), stateBufHeader, sizeof(stateBufHeader));		// Packet Header
			}
			else {
				memcpy((uint8_t *)(pTxMem+1), stateBufHeader, sizeof(stateBufHeader));		// Packet Header
				memcpy((uint8_t *)(pTxMem+1), stateBufHeader, sizeof(stateBufHeader));		// Packet Header				
		...
		if(TSTAT&0x80) {
			...
			packetInfo = rs422if.sendDataInfo[rs422if.sendDataTail];
			rs422if.sendDataTail = (++rs422if.sendDataTail) & 0x1f;

			switch(packetInfo) {
			case RS422_BUF_STATE_DATA:
				FPGA_WR_RS422_TX_FLAG_CLEAR;
				pTxMem[0] = STATE_REQUEST_CMD;			// FPGA에서 자동으로 결정된다. (0xB0)
				memcpy((uint8_t *)(pTxMem+1), stateBufHeader, sizeof(stateBufHeader));		// Packet Header
// - 보낼 수 있을 때 그때 데이터를 보낸다.
// 현재 저장된 데이터를 보내자 
//				memcpy((uint8_t *)(pTxMem+7), &rs422_stateBuf[rs422if.stateDataBufTail], sizeof(rs422_stateBuf[rs422if.stateDataBufTail]));
//				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(rs422_stateBuf[rs422if.stateDataBufTail]) >> 2) + 2); 	// tx_len : uint8_t --> uint32_t ,	Head , Tail

				LcasState.sample.pos = devState.curSample.pos;
				LcasState.injectVial.pos = devState.injectVial.pos;
				memcpy((uint8_t *)(pTxMem+7), &LcasState, sizeof(LcasState));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(LcasState) >> 2) + 2);	// tx_len : uint8_t --> uint32_t ,	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
				rs422if.stateDataBufTail = (++rs422if.stateDataBufTail) & 0x7;
				break;
			case RS422_BUF_DIAG_DATA:
				FPGA_WR_RS422_TX_FLAG_CLEAR;
				pTxMem[0] = STATE_REQUEST_CMD;			// FPGA에서 자동으로 결정된다. (0xB0)
				memcpy((uint8_t *)(pTxMem+1), diagDataBufHeader, sizeof(diagDataBufHeader));		// Packet Header
				memcpy((uint8_t *)(pTxMem+7), &rs422_diagDataBuf[rs422if.diagDataBufTail], sizeof(LCAS_DIAG_DATA_t));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(LCAS_DIAG_DATA_t) >> 2) + 2); 	// tx_len : uint8_t --> uint32_t ,	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
				rs422if.diagDataBufTail = (++rs422if.diagDataBufTail) & 0x7;
				break;
#if RS422_MY_ID	== RS422_ID_UVD				
			case RS422_BUF_ACQ_DATA:
				FPGA_WR_RS422_TX_FLAG_CLEAR;
				pTxMem[0] = STATE_REQUEST_CMD;			// FPGA에서 자동으로 결정된다. (0xB0)
				memcpy((uint8_t *)(pTxMem+1), acqDataBufHeader, sizeof(acqDataBufHeader));		// Packet Header
				memcpy((uint8_t *)(pTxMem+7), &rs422_acqDataBuf[rs422if.acqDataBufTail], sizeof(YL9020_UVD_ACQDATA));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(YL9020_UVD_ACQDATA) >> 2) + 2); 	// tx_len : uint8_t --> uint32_t ,	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
				rs422if.acqDataBufTail = (++rs422if.acqDataBufTail) & 0x3;
				break;
			case RS422_BUF_SCAN_DATA:
				FPGA_WR_RS422_TX_FLAG_CLEAR;
				pTxMem[0] = STATE_REQUEST_CMD;			// FPGA에서 자동으로 결정된다. (0xB0)
				memcpy((uint8_t *)(pTxMem+1), scanDataBufHeader, sizeof(scanDataBufHeader));		// Packet Header
				memcpy((uint8_t *)(pTxMem+7), &rs422_scanDataBuf[rs422if.scanDataBufTail], sizeof(YL9020_UVD_SCANDATA));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(YL9020_UVD_SCANDATA) >> 2) + 2);	// tx_len : uint8_t --> uint32_t ,	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
				rs422if.scanDataBufTail = (++rs422if.scanDataBufTail) & 0x7;
				break;
#endif				
//			case RS422_BUF_SELFMSG_DATA:				break;
			default:
				break;
			}
...
#define SEND_LCAS_PACKET(Code,full_data,entry_data)	{ \
	if(CommDevice == COMMDEV_TCP_IP) { \
		MAKE_LCAS_PACKET(Code,full_data,entry_data); \
		TCP_WRITE_PACKET; \
	} \
	else if(CommDevice == COMMDEV_RS422) { \
...
#define MAKE_LCAS_PACKET(Code,full_data,entry_data) { \
	...
	memcpy(rs422_selfMsgBuf[index], selfMsg, sizeof(selfMsg));
...
#if 0
//int r_dequeue(char* pBuffer, int offset, int size)
...
//		return 0;
	...
	bf->Head = (bf->Head + size) % r_get_buffer_size();
