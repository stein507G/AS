...
step = devSet.adjust.value[MOTOR_VAL] * ADJUST_STEP_UNIT_VAL;
	if(step >= 0) {
		stepControl(MOTOR_VAL, HOME_DIR_VAL | CMD_START, devSet.chkHomeSpeed[MOTOR_VAL], step);
	}
	else {
		stepControl(MOTOR_VAL, R_HOME_DIR_VAL | CMD_START, devSet.chkHomeSpeed[MOTOR_VAL], -step);
	}
					...
					if(System_ticks > check_time_cnt) { // 시간 초과 
						errorFunction(SELF_ERROR_HOME_CHK_VAL);
					}
				}
			}
