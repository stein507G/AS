...
{
	// selfMsgBufHeader[24]
	((LCAS_PACKET *)selfMsgBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(selfMsg);
	...
	((LCAS_PACKET *)selfMsgBufHeader)->nSlotSize = sizeof(selfMsg);
	...
	((LCAS_PACKET *)scanDataBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(YL9020_UVD_SCANDATA);
	((LCAS_PACKET *)scanDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((LCAS_PACKET *)scanDataBufHeader)->nPacketCode = (PACKCODE_YL9020_SCANDATA);
	((LCAS_PACKET *)scanDataBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)scanDataBufHeader)->nSlotOffset = 0;
	((LCAS_PACKET *)scanDataBufHeader)->nSlotSize = sizeof(YL9020_UVD_SCANDATA);

	// acqDataBufHeader[24]
	((LCAS_PACKET *)acqDataBufHeader)->nPacketLength = sizeof(LCAS_PACKET) + sizeof(YL9020_UVD_ACQDATA);
	((LCAS_PACKET *)acqDataBufHeader)->nDeviceID = (PACKET_DEVICE_ID_UVD_PACKET);
	((LCAS_PACKET *)acqDataBufHeader)->nPacketCode = (PACKCODE_YL9020_ACQDATA);
	((LCAS_PACKET *)acqDataBufHeader)->nEventIndex = 0;
	((LCAS_PACKET *)acqDataBufHeader)->nSlotOffset = 0;
	((LCAS_PACKET *)acqDataBufHeader)->nSlotSize = sizeof(YL9020_UVD_ACQDATA);
#endif
...
int send_SelfMessage(unsigned char ucMessage, unsigned char ucNewValue)
{
	...
	if(CommDevice == COMMDEV_TCP_IP) {
		...
		memcpy(rs422_selfMsgBuf[rs422if.selfMsgBufHead], selfMsg, sizeof(selfMsg));
		rs422if.selfMsgBufHead = (++rs422if.selfMsgBufHead) & 0x7;
	}
...
//if(rs422if.TSTAT == 0)
	...
	if(rs422if.selfMsgBufHead != rs422if.selfMsgBufTail) {
		...
		if(TSTAT&0x80) {
			...
			memcpy((uint8_t *)(pTxMem+7), &rs422_selfMsgBuf[rs422if.selfMsgBufTail], sizeof(rs422_selfMsgBuf[rs422if.selfMsgBufTail]));
			FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(rs422_selfMsgBuf[rs422if.selfMsgBufTail]) >> 2) + 2); 	//packetHeader(6)  tx_len : uint8_t --> uint32_t , 	Head(1) , Tail(1)
		...
		if(TSTAT&0x80) {
			...
			case RS422_BUF_DIAG_DATA:
				...
				memcpy((uint8_t *)(pTxMem+7), &rs422_diagDataBuf[rs422if.diagDataBufTail], sizeof(rs422_diagDataBuf[rs422if.diagDataBufTail]));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(rs422_diagDataBuf[rs422if.diagDataBufTail]) >> 2) + 2); 	// tx_len : uint8_t --> uint32_t , 	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
			...
			case RS422_BUF_ACQ_DATA:
				...
				memcpy((uint8_t *)(pTxMem+7), &rs422_acqDataBuf[rs422if.acqDataBufTail], sizeof(rs422_acqDataBuf[rs422if.acqDataBufTail]));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(rs422_acqDataBuf[rs422if.acqDataBufTail]) >> 2) + 2); 	// tx_len : uint8_t --> uint32_t , 	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
			...
			case RS422_BUF_SCAN_DATA:
				...
				memcpy((uint8_t *)(pTxMem+7), &rs422_scanDataBuf[rs422if.scanDataBufTail], sizeof(rs422_scanDataBuf[rs422if.scanDataBufTail]));
				FPGA_WR_RS422_TX_LENGTH(6 + (sizeof(rs422_scanDataBuf[rs422if.scanDataBufTail]) >> 2) + 2); 	// tx_len : uint8_t --> uint32_t , 	Head , Tail
				FPGA_WR_RS422_TX_DATA_SAVED;			// 보낼 데이터가 저장되었다.
