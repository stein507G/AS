...
#if 0
typedef struct _INFORMATION_t
{
	unsigned int setFlag;	// bit가 1이면 해당 항목 저장함.

	char cSerialNO[20];							// ex) 								--- (bit 0)
	...
	unsigned char cModel[10];	// cModel[32]		// RD	SYSTEM_MODEL		"YI9650"		-- AS / OVEN  연결시 검사 
	unsigned char cFirmwareVersion[2];	// RD 06/11  	버전	 "6.11"
	unsigned char cMachineVersion[2];		// RD 1/3  		버전	 "1.3"

	unsigned char cIPAddress[4];	// ex) "10.10.10.52"		--- (bit 3)
	unsigned char cGateway[4];		// ex) "10.10.10.1"			--- (bit 3)
	unsigned char cNetmask[4];		// ex) "255.255.255.0"	--- (bit 3)
	unsigned int PortNo;					// ex) "4242"						--- (bit 3)
	unsigned char cMacAddress[6];	// "00112233445A" --- (bit 4)
	...
	unsigned char TempConModule;	// AS 온도제어모듈 유무	--- (bit 6)
