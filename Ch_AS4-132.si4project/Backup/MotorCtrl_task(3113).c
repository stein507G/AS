...
// Syringe Motor는 엔코더가 반대로 장착되어 있다. 출력되는 값이 반대임. - FPGA에서 옵션사항으로 추가해야 함. 
				CMD_CHK_DIR_OPTION_DIR_SYR = 0x01; 	// DIR_IN 
#endif
			}
			break;
		case 2:
...
//*((uint32_t volatile *)(BASEADDR_MOTOR_X_Z + (0x301 << 2));
//CMD_SEL_MOTOR(MOTOR_X); // 이동전 실행해야
#define CMD_EN_STEP_COUNT(WHO)	*((uint32_t volatile *)(devSet.selStateAddr[WHO])) = WHO;
#define CMD_CLR_STEP_COUNT(WHO)	*((uint32_t volatile *)(devSet.selStateAddr[WHO])) = WHO;


#define  SEL_STATE_ADDR_MOTOR_X_Z				(BASEADDR_MOTOR_X_Z + S7_SEL_STATE)

*((uint32_t volatile *)(BASEADDR_MOTOR_X_Z + (0x301 << 2));	// CMD_CLR_STEP_COUNT
*((uint32_t volatile *)(BASEADDR_MOTOR_X_Z + (0x302 << 2));	// CMD_EN_STEP_COUNT


*((uint32_t volatile *)(BASEADDR_MOTOR_X_Z + (0x1 << 2));	// read stepCount
*/
...
#if CHECK_ENCODER_DIRECTION
/*
	devState.chk_Dir[MOTOR_Z] = READ_CHK_DIR_Z;
	devState.chk_Dir[MOTOR_X] = (devState.chk_Dir[MOTOR_Z] >> 16) & 0xFFFF;

	devState.chk_Dir[MOTOR_Y] = READ_CHK_DIR_Y;
	devState.chk_Dir[MOTOR_SYR] = (devState.chk_Dir[MOTOR_Y] >> 16) & 0xFFFF;
*/
	devState.chk_Dir[MOTOR_Z] = READ_CHK_DIR_Z;
	devState.chk_Dir[MOTOR_X] = READ_CHK_DIR_X;
	devState.chk_Dir[MOTOR_Y] = READ_CHK_DIR_Y;
	devState.chk_Dir[MOTOR_SYR] = READ_CHK_DIR_SYR;
