...
typedef struct _INFORMATION_t
{
	...
	unsigned char TempConModule;	// AS // 온도제어 모듈 유무 			-------- (bit 5)
	
	unsigned char LED_onoff;		// AS										-------- (bit 6)
	unsigned char Door_check;		// AS / Oven						-------- (bit 6)
	unsigned char Leak_check;		// AS / Oven / UVD			-------- (bit 6)
	unsigned char Buzzer_onoff;	// AS / Oven / UVD			-------- (bit 6)
