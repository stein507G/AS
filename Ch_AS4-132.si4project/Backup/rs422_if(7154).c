...
//	memcpy((uint8_t *)(pTxMem_byte), (const void*)&bf->Data[bf->Head], size);
...
//		memcpy((uint8_t *)(pTxMem_byte + 4), (const void*)&bf->Data[bf->Head], size);

//memcpy(&pBuffer[0], (const void*)&bf->Data[bf->Head], size);
//		memcpy(&pTxMem[1], (const void*)&bf->Data[bf->Head], size);
//memcpy((&pTxMem[1]), &pBuffer[0], size);

//		memmove((&pTxMem[1]), &pBuffer[0], size);
...
for(int i=0 ; i < size ;i++) {
	...
	iprintf("[%d]%x %x\n",i, pTxMem_byte[i], pAddr[i]), pData[i];
}
