...
#if RS422_ENABLE
	if(devState.rs422if_packet != YES) {
...
// 20220721
	if(tmp.cSerialNo[0] == 'F' && tmp.cSerialNo[1] == 'F') {
		memcpy(as_INFO.cSerialNO, &tmp.cSerialNo[2], 14);
		as_INFO.cSerialNO[16] = '\0';

		memcpy(sysInform.cSerialNo, &tmp.cSerialNo[2], 14); 
		sysInform.cSerialNo[16] = '\0';

//printf("as_INFO.cSerialNo = %s \n",as_INFO.cSerialNO);

/*
for(int i=0; i<4; i++) 
{
	as_INFO.cInstallDate[i] = tmp.cInstallDate[i];
}
*/
sysInform.cInstallDate.year = tmp.cInstallDate.year;
sysInform.cInstallDate.month = tmp.cInstallDate.month;
...
as_INFO.cInstallDate[3] = 20;
as_INFO.cInstallDate[2] = tmp.cInstallDate.year;
as_INFO.cInstallDate[1] = tmp.cInstallDate.month;
as_INFO.cInstallDate[0] = tmp.cInstallDate.date;

//		save_SerialNo_InstallDate_Envm();
...
int save_EEPROM_sysConfig_data()
{
	...
	memcpy(&data[1], &(sysConfig) , sizeof(sysConfig));
#if STORE_INFORMATION_ENVM	
//	NVM_write((ENVM_SYSTEM_CONFIG_DATA_ADDR), &data, (ENVM_SYSTEM_CONFIG_DATA_LEN + 1), NVM_DO_NOT_LOCK_PAGE);
	NVM_write((ENVM_SYSTEM_CONFIG_DATA_ADDR), data, (ENVM_SYSTEM_CONFIG_DATA_LEN + 1), NVM_DO_NOT_LOCK_PAGE);
...
void convert_TempCal_eeprom2envm()
{
	uint8_t data[21];

	eepromRead_Data(TEMP_CAL_DATA_SAVE,(INFO_TEMP_CAL_ENVM_LEN+1),data);
	if(data[0] == DATA_SAVE_FLAG) {
//		NVM_write((INFO_TEMP_CAL_ENVM_ADDR), &data, (INFO_TEMP_CAL_ENVM_LEN + 1), NVM_DO_NOT_LOCK_PAGE);	
		NVM_write((INFO_TEMP_CAL_ENVM_ADDR), data, (INFO_TEMP_CAL_ENVM_LEN + 1), NVM_DO_NOT_LOCK_PAGE);	
		dp("convert_TempCal_eeprom2envm() \n");
	}
}
...
void tempConstCalc(unsigned char act)
{
	...
	//		case 0: // load
			case TEMP_CAL_LOAD:
				MSS_NVM_read( (uint8_t *)INFO_TEMP_CAL_ENVM_ADDR, &saveFlag, 1);
...
//					MSS_NVM_read( (uint8_t *)INFO_TEMP_CAL_ENVM_ADDR, &tempCalData.meaTemp1, 8);
					MSS_NVM_read( (uint8_t *)INFO_TEMP_CAL_ENVM_ADDR, (uint8_t *)(&tempCalData.meaTemp1), 8);					
	...
	dfp("Cal Temp Load Error!!!\n");					
				...
				as_INFO.fCalTemp[0] = tempCalData.meaTemp1;
				as_INFO.fCalTemp[1] = tempCalData.meaTemp2; 			
...
//				NVM_write((INFO_TEMP_CAL_ENVM_ADDR), &data, (INFO_TEMP_CAL_ENVM_LEN + 1), NVM_DO_NOT_LOCK_PAGE);	
			...
			case TEMP_CAL_RESET:
				...
				saveFlag = 0x00;
				NVM_write((INFO_TEMP_CAL_ENVM_ADDR), &saveFlag,  1, NVM_DO_NOT_LOCK_PAGE);		
				break;
		}
...
dp("airSegmentVol %d \n",tmp.airSegmentVol);
*/
diprintf("SELF_ERROR_COMMAND_UNKNOWN\n");
		return RE_ERR;
	}

diprintf("PACKCODE_LCAS_RUN_SETUP === injectionVolume %f tmp.washNeedleBeforeInj %d\n",tmp.injectionVolume,tmp.washNeedleBeforeInj);
...
dprintf("tmp.mixTable[%d].action = %d err = %d ret = %d\n",i,action,err,ret);			
...
dprintf("PACKCODE_LCAS_COMMAND cmd=%d sub1=%d sub2=%d\n LcasState.state=%d devState.command=%d\r\n"
			...
			case ADJUST_LEFT:	// CCW
				switch(part) {
					case ADJUST_NEEDLE_XY:
						if(devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_X] >= ADJUST_VALUE_MAX) {
...
//						++devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_X];
					...
					case ADJUST_VALVE_POS:
						if(devSet.adjustTmp.value[ADJUST_VALVE_X] >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
					...
					case ADJUST_INJECTOR_POS:
						if(devSet.adjustTmp.value[ADJUST_INJECTOR_X] >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
					...
					case ADJUST_WASHING_PORT:
						if(devSet.adjustTmp.wash_x >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
						goX(ADJUST_STEP_UNIT_X, 0, ACT_MOVE_STEP_RELATE);
					...
					case ADJUST_WASTE_PORT:
						if(devSet.adjustTmp.waste_x >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
						goX(ADJUST_STEP_UNIT_X, 0, ACT_MOVE_STEP_RELATE);
					...
					case ADJUST_VIAL_LEFT:
						if(devSet.adjustTmp.leftVial_x[sysConfig.trayLeft] >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
						goX(ADJUST_STEP_UNIT_X, 0, ACT_MOVE_STEP_RELATE);
					...
					case ADJUST_VIAL_RIGHT:
						if(devSet.adjustTmp.rightVial_x[sysConfig.trayRight] >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
						goX(ADJUST_STEP_UNIT_X, 0, ACT_MOVE_STEP_RELATE);
					...
					case ADJUST_VIAL_HEIGHT_XY:
						if(devSet.adjustTmp.vialHeight_x >= ADJUST_VALUE_MAX) {
							send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
							break;
						}
						goX(ADJUST_STEP_UNIT_X, 0, ACT_MOVE_STEP_RELATE);
	    ...
	    case ADJUST_LEFT_X2:	// CCW X2
			switch(part) {
				case ADJUST_NEEDLE_XY:
					if(devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_X] >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
				...
				case ADJUST_SYRINGE:						break;						
				case ADJUST_VALVE_POS:
					if(devSet.adjustTmp.value[ADJUST_VALVE_X] >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					...
					break;
				case ADJUST_INJECTOR_POS:
					if(devSet.adjustTmp.value[ADJUST_INJECTOR_X] >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					...
					break;
				case ADJUST_WASHING_PORT:
					if(devSet.adjustTmp.wash_x >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goX(ADJUST_STEP_UNIT_X * ADJUST_STEP_UNIT_SCALE_X, 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_WASTE_PORT:
					if(devSet.adjustTmp.waste_x >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goX(ADJUST_STEP_UNIT_X * ADJUST_STEP_UNIT_SCALE_X, 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_VIAL_LEFT:
					if(devSet.adjustTmp.leftVial_x[sysConfig.trayLeft] >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goX(ADJUST_STEP_UNIT_X * ADJUST_STEP_UNIT_SCALE_X, 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_VIAL_RIGHT:
					if(devSet.adjustTmp.rightVial_x[sysConfig.trayRight] >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goX(ADJUST_STEP_UNIT_X * ADJUST_STEP_UNIT_SCALE_X, 0, ACT_MOVE_STEP_RELATE);
				...
				case ADJUST_VIAL_HEIGHT:
					...
					break;
				case ADJUST_VIAL_HEIGHT_XY:
					if(devSet.adjustTmp.vialHeight_x >= ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goX(ADJUST_STEP_UNIT_X * ADJUST_STEP_UNIT_SCALE_X, 0, ACT_MOVE_STEP_RELATE);
	    ...
	    case ADJUST_UP:
			switch(part) {
				case ADJUST_NEEDLE_XY:
					...
					break;
				case ADJUST_NEEDLE_HEIGHT:
					if(devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_Z] <= -ADJUST_VALUE__NEEDLE_H_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goZ((-ADJUST_STEP_UNIT_Z), ACT_MOVE_STEP_RELATE);			
					...
					break;
				case ADJUST_SYRINGE:
					if(devSet.adjustTmp.value[ADJUST_SYRINGE_Z] <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
				...
				case ADJUST_INJECTOR_POS:
					break;
				case ADJUST_VIAL_HEIGHT:
					if(devSet.adjustTmp.vialHeight <= -ADJUST_VALUE__NEEDLE_H_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goZ((-ADJUST_STEP_UNIT_Z), ACT_MOVE_STEP_RELATE);
	    ...
	    case ADJUST_UP_X2:
			switch(part) {
				case ADJUST_NEEDLE_XY:
					...
					break;
				case ADJUST_NEEDLE_HEIGHT:
					if(devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_Z] <= -ADJUST_VALUE__NEEDLE_H_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goZ((-ADJUST_STEP_UNIT_Z) * ADJUST_STEP_UNIT_SCALE_Z, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_SYRINGE:
					if(devSet.adjustTmp.value[ADJUST_SYRINGE_Z] <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
				...
				case ADJUST_INJECTOR_POS:
					break;
				case ADJUST_VIAL_HEIGHT:
					if(devSet.adjustTmp.vialHeight <= -ADJUST_VALUE__NEEDLE_H_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goZ((-ADJUST_STEP_UNIT_Z) * ADJUST_STEP_UNIT_SCALE_Z, ACT_MOVE_STEP_RELATE);
...
//						goZ((-ADJUST_STEP_UNIT_Z), ACT_MOVE_STEP_RELATE);										
	    ...
	    case ADJUST_BACK:		// Back
			switch(part) {
				case ADJUST_NEEDLE_XY:
					if(devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_Y] <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
				...
				case ADJUST_INJECTOR_POS:
					break;
				case ADJUST_WASHING_PORT:
					if(devSet.adjustTmp.wash_y <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y), 0, ACT_MOVE_STEP_RELATE);
					...
					break;						
				case ADJUST_WASTE_PORT:
					if(devSet.adjustTmp.waste_y <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y), 0, ACT_MOVE_STEP_RELATE);
					...
					break;						
				case ADJUST_VIAL_LEFT:
					if(devSet.adjustTmp.leftVial_y[sysConfig.trayLeft] <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y), 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_VIAL_RIGHT:
					if(devSet.adjustTmp.rightVial_y[sysConfig.trayRight] <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y), 0, ACT_MOVE_STEP_RELATE);
				...
				case ADJUST_VIAL_HEIGHT:
					...
					break;
				case ADJUST_VIAL_HEIGHT_XY:
					if(devSet.adjustTmp.vialHeight_y <= -ADJUST_VALUE_MAX) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y), 0, ACT_MOVE_STEP_RELATE);
	    ...
	    case ADJUST_BACK_X2:	// Back X2
			switch(part) {
				case ADJUST_NEEDLE_XY:
					if(devSet.adjustTmp.value[ADJUST_INJ_PORT_VALUE_Y] <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
				...
				case ADJUST_INJECTOR_POS:
					break;
				case ADJUST_WASHING_PORT:
					if(devSet.adjustTmp.wash_y <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y) * ADJUST_STEP_UNIT_SCALE_Y, 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_WASTE_PORT:
					if(devSet.adjustTmp.waste_y <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y) * ADJUST_STEP_UNIT_SCALE_Y, 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_VIAL_LEFT:
					if(devSet.adjustTmp.leftVial_y[sysConfig.trayLeft] <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y) * ADJUST_STEP_UNIT_SCALE_Y, 0, ACT_MOVE_STEP_RELATE);
					...
					break;
				case ADJUST_VIAL_RIGHT:
					if(devSet.adjustTmp.rightVial_y[sysConfig.trayRight] <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y) * ADJUST_STEP_UNIT_SCALE_Y, 0, ACT_MOVE_STEP_RELATE);
				...
				case ADJUST_VIAL_HEIGHT:
					...
					break;
				case ADJUST_VIAL_HEIGHT_XY:
					if(devSet.adjustTmp.vialHeight_y <= -ADJUST_VALUE_LIMIT) {
						send_SelfMessage(SELF_COMMAND_REJECT,ADJUST_VALUE_ERROR_0);
						break;
					}
					goY((-ADJUST_STEP_UNIT_Y) * ADJUST_STEP_UNIT_SCALE_Y, 0, ACT_MOVE_STEP_RELATE);
...
dp("===========================\n devSet.adjust_2nd.saveFlag = %d devSet.adjust_2nd.value = %d\n",devSet.adjust_2nd.saveFlag[i],devSet.adjust_2nd.value[i]);
					...
					nvmWrite_Data_Adjust(HOME_ADJUST_DATA_STARTADDR,sizeof(devSet.adjust) + 1,devSet.adjust.saveFlag);	
					...
					ackPacket = NO;		
					nvmWrite_Data_Adjust_2nd(HOME_ADJUST_2ND_STARTADDR,sizeof(devSet.adjust) + 1,devSet.adjust_2nd.saveFlag);																		
...
//	    	devSet.adjust.value[MOTOR_Z] += STEP_MORE_ADJUST_VALUE_Z;
	    ...
	    case ADJUST_INIT_VALUE:
			...
			nvmWrite_Data_Adjust_2nd(HOME_ADJUST_2ND_STARTADDR,sizeof(devSet.adjust_2nd) + 1,devSet.adjust_2nd.saveFlag);
...
//			devSet.adjustCurMotor = service.subCommand1;		// part
			devSet.adjustCurMotor = part; // service.subCommand1;		// part
...
//=======================================================================
#if RS422_ENABLE
	if( (devState.rs422if_packet == YES && devState.rs422if_connected == NO) || (devState.rs422if_packet != YES && devState.connected == NO)) {			
#else
	if(	devState.connected == NO) {
#endif
		switch(nPacketCode) {
			case PACKCODE_LCAS_SYSTEM_INFORM:
				if(IS_REQUEST_PACKET(pData)) {			dfp("XXXXXXXXIS_REQUEST_PACKET--PACKCODE_LCAS_SYSTEM_INFORM\n");				
					SEND_RESPONSE_LCAS_PACKET(sysInform,pData);
				}
				else if(IS_ACK_PACKET(pData)){	dfp("ack--PACKCODE_LCAS_SYSTEM_INFORM\n");				}
				else {	dfp("rcv--PACKCODE_LCAS_SYSTEM_INFORM\n");
	// 에러일 경우는 tcp_close()
...
#endif
#if 1
	if(MotorCtrlSet.bCtrlEnable[MOTOR_X]) {
		dp("[MOTOR X] ==================== \n");
		dp("bDir 		= 0x%02x \n",MotorCtrlSet.bDir[MOTOR_X]);	
		dp("nStep		= %d \n",MotorCtrlSet.nStep[MOTOR_X]);
		dp("nSpeedMin	= %d \n",MotorCtrlSet.nSpeedMin[MOTOR_X]);
		dp("nSpeedMax	= %d \n",MotorCtrlSet.nSpeedMax[MOTOR_X]);
	}
	if(MotorCtrlSet.bCtrlEnable[MOTOR_Y]) {
		dp("[MOTOR Y] ==================== \n");
		dp("bDir		= 0x%02x \n",MotorCtrlSet.bDir[MOTOR_Y]);	
		dp("nStep		= %d \n",MotorCtrlSet.nStep[MOTOR_Y]);
		dp("nSpeedMin	= %d \n",MotorCtrlSet.nSpeedMin[MOTOR_Y]);
		dp("nSpeedMax	= %d \n",MotorCtrlSet.nSpeedMax[MOTOR_Y]);
	}
	if(MotorCtrlSet.bCtrlEnable[MOTOR_Z]) {
		dp("[MOTOR Z] ==================== \n");
		dp("bDir		= 0x%02x \n",MotorCtrlSet.bDir[MOTOR_Z]);	
		dp("nStep		= %d \n",MotorCtrlSet.nStep[MOTOR_Z]);
		dp("nSpeedMin	= %d \n",MotorCtrlSet.nSpeedMin[MOTOR_Z]);
		dp("nSpeedMax	= %d \n",MotorCtrlSet.nSpeedMax[MOTOR_Z]);
	}
	if(MotorCtrlSet.bCtrlEnable[MOTOR_SYR]) {
		dp("[Syringe] ==================== \n");
		dp("bDir		= 0x%02x \n",MotorCtrlSet.bDir[MOTOR_SYR]);	
		dp("nStep		= %d \n",MotorCtrlSet.nStep[MOTOR_SYR]);
		dp("nSpeedMin	= %d \n",MotorCtrlSet.nSpeedMin[MOTOR_SYR]);
		dp("nSpeedMax	= %d \n",MotorCtrlSet.nSpeedMax[MOTOR_SYR]);
	}
	if(MotorCtrlSet.bCtrlEnable[MOTOR_VAL]) {
		dp("[Valve] ==================== \n");
		dp("bDir		= 0x%02x \n",MotorCtrlSet.bDir[MOTOR_VAL]);	
		dp("nStep		= %d \n",MotorCtrlSet.nStep[MOTOR_VAL]);
		dp("nSpeedMin	= %d \n",MotorCtrlSet.nSpeedMin[MOTOR_VAL]);
		dp("nSpeedMax	= %d \n",MotorCtrlSet.nSpeedMax[MOTOR_VAL]);
	}
	if(MotorCtrlSet.bCtrlEnable[MOTOR_INJ]) {
		dp("[Injector] ==================== \n");
		dp("bDir		= 0x%02x \n",MotorCtrlSet.bDir[MOTOR_INJ]);	
		dp("nStep		= %d \n",MotorCtrlSet.nStep[MOTOR_INJ]);
		dp("nSpeedMin	= %d \n",MotorCtrlSet.nSpeedMin[MOTOR_INJ]);
		dp("nSpeedMax	= %d \n",MotorCtrlSet.nSpeedMax[MOTOR_INJ]);
	}
#endif
...
//			stepCtrlCmd(MOTOR_X , MotorCtrlSet.bDir[5] | CMD_STOP | MotorCtrlSet.bInfinite[5]); // CMD_STOP 사용안함.
			break;
...
#if STORE_INFORMATION_ENVM	
		NVM_write((INFO_SPECIAL_ENVM_ADDR), &data[0], (INFO_SPECIAL_ENVM_LEN), NVM_DO_NOT_LOCK_PAGE);
#else
	eepromWrite_Data((SPECIAL_DATA_SAVE), sizeof(special) + 1, &(data[0]));
#endif
...
//#if PHOTO_SENSOR_TEST
	...
	else if(devState.rs422if_connected) {
