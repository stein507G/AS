...
//		memcpy(as_INFO.cInstallDate, data[18], 3);
		...
		memcpy(&idata[0] ,&oven_INFO.cSerialNO[0], sizeof(oven_INFO.cSerialNO));
...
//		OVEN_TEMPCAL_t tempCal;
		...
		if(devState.envmDataAvailable == RE_ERR) { // 변환해야
			eeprom2envm_data();
			eeprom2envm = YES;
		}
		else { // 사용중이다.  //		if(ovenStt.envmDataAvailable) {
		// =================== Save Flag
	...
	//		saveEnvmFlag[FLAG_MAC_ADDR] = data[2];
		...
		// =================== IP ADDR
			if(ovenStt.IP_switch == 7) { // default ip사용	// DEFAULT_IPADDRESS - 저장
				init_IpAddr(0);
				saveIP = YES;
...
//	memcpy(&idata[0] ,&oven_INFO.cInstallDate[0], sizeof(oven_INFO.cInstallDate));
	ret = NVM_write(INFO_INSTALL_DATE_ENVM_ADDR, &oven_INFO.cInstallDate[0], INFO_INSTALL_DATE_ENVM_LEN, NVM_DO_NOT_LOCK_PAGE);
	return 0;
}
...
int save_IPADDR()
{
	...
	addr = INFO_IP_ADDR_ENVM_ADDR;
	len = INFO_IP_ADDR_ENVM_LEN;
	memcpy(&idata[0], oven_INFO.cIPAddress, 4);
	memcpy(&idata[4], oven_INFO.cGateway, 4);
