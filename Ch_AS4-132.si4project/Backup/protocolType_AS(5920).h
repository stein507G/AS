...
typedef struct _INFORMATION_t
{
	...
	unsigned char cModel[10];							// RD	SYSTEM_MODEL				"YI9650"
	unsigned char cFirmwareVersion[3];		// RD 01/01/03  버전		 ex) "1.1.3"
	unsigned char cMachineVersion[3];			// RD 01/03/08  버전		 ex) "1.3.8"
	...
	unsigned int cPortNo;					// ex) "4242"							-------- (bit 3)

	unsigned char cMacAddress[6];	// ex) "00:11:22:33:44:55"	-------- (bit 4)
	...
	unsigned char TempConModule;	// AS // 온도제어 모듈 유무 			-------- (bit 6)

	unsigned char setTemp;			// (1)35 / (2)60		-------- (bit 7)
	float fCalTemp[2];					// 									-------- (bit 7)

	unsigned char cdummy1[4];		// ex) 			-------- (bit 8)
	unsigned char cdummy2[4];		// ex) 			-------- (bit 9)
