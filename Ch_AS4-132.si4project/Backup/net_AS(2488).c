...
int send_State_Blank()
{
	if(rs422if.connected) {
		memcpy(rs422_stateBuf[rs422if.stateDataBufHead], LcasState, sizeof(LcasState));
		rs422if.stateDataBufHead = (++rs422if.stateDataBufHead) & 0x7;
...
//		| ((devState.btMotorState[MOTOR_Y] >> 1) & 0x00000038)
	...
	SEND_LCAS_PACKET(PACKCODE_LCAS_DIAG_DATA, diagData, diagData);
		...
		memcpy(rs422_diagDataBuf[rs422if.diagDataBufHead], diagData, sizeof(diagData));
		rs422if.diagDataBufHead = (++rs422if.diagDataBufHead) & 0x7;
	}
