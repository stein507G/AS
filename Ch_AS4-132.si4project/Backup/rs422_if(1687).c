...
//		memcpy((uint8_t *)(pTxMem+1), (const void*)&bf->Data[bf->Head], size);
		ret = memcpy(&pTxMem[1], (const void*)&bf->Data[bf->Head], size);
//pTxMem[2] = 0x12345678;			// FPGA에서 자동으로 결정된다. (0xB0)
	}

	iprintf("r_save_TxMem H=%d[%d] T=%d[%d] len=%d[%d][%d] nPacketCode=%x ret=%d\n",tx.Head ,bf->Head,tx.Tail, bf->Tail, pTxMem[1], bf->Data[bf->Head], size,pTxMem[2],ret);
