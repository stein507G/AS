...
#if 0 //READY_CHECK_RETRY_FPGA
			else {
				if(Chk_MotorErr_X()) subMenu_rcrx++;
	...
	switch(option) {
		case ACT_XY_POS:
			switch(pVial->uPos.tray) {
				case TRAY_LEFT:
//					act = xyzCoord.vialStep_L_X[pos];
//					calX = devSet.adjust.leftVial_x[sysConfig.trayLeft]
//					y_pos = pVial->uPos.x; //pVial->uPos.etc;
//					Ycount = devSet.trayLeft_colNo; // devSet.trayLeft_colNo = TRAY60_COUNT_Y;
//					act = xyzCoord.vialStep_L_X[pos] + calX * y_pos / Ycount;
//					pos = pVial->uPos.x;
///					devState.mActSet[MOTOR_X].pos = pVial->uPos.x; //pos;	// vial 좌표 a~e (a1 .. e12)
...
///					devState.mActSet[MOTOR_X].pos = pVial->uPos.x; //pos;	// vial 좌표 a~e (a1 .. e12)
				...
				case TRAY_ETC:
//					pos = pVial->uPos.etc;
///					devState.mActSet[MOTOR_X].pos = pVial->uPos.etc;
...
///	devState.mActSet[MOTOR_Y].Tray = pVial->uPos.tray; //Tray;
	...
	switch(option) {
		case ACT_XY_POS:
			switch(pVial->uPos.tray) {
				case TRAY_LEFT:
///					devState.mActSet[MOTOR_Y].pos = pVial->uPos.y; //pos;
...
///					devState.mActSet[MOTOR_Y].pos = pVial->uPos.y; //pos;
					...
					break;
				case TRAY_ETC:
///					devState.mActSet[MOTOR_Y].pos = pVial->uPos.etc; //pos;
...
//dprintf("===	goSyr pos=%f curStep=%f cur=%f act=%f step=%d option=%d\n", pos,devState.curStep[MOTOR_SYR], cur, act, step, option);	
	...
	if(step == 0) {
		...
		printf("## MOTOR_SYR CMD_CHK_DIR_START chk_DirS=%08x cur=%f act=%f step=%d\n",devState.chk_Dir[MOTOR_SYR], cur, act, step);
		}
#endif
