...
#if 0		
		else {	// 초기화 default - rotory sw - 저장안함.
...
/*	
		...
		my_mac[5] = ETHERNET_CONF_ETHADDR5 + ovenStt.IP_switch; 		

		addr = INFO_MACADDR_ENVM_ADDR;
		len = INFO_MACADDR_ENVM_LEN;
		NVM_write(addr, my_mac, len, NVM_DO_NOT_LOCK_PAGE);

	memcpy(oven_INFO.cMacAddress, my_mac, INFO_MACADDR_ENVM_LEN);

		iprintf("[load MAC] ");
	}
	else {	// 초기화 // 디폴트 값이 적용된다. - IP switch 적용
