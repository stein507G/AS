...
check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_VAL; // #if HOME_FIND_TIME_CHECK_VAL	// 일정시간이후 homeChkEnd 안되면 에러추가			
		...
		case 2:
			...
			else if(System_ticks > check_time_cnt) { // 시간 초과 
				errorFunction(SELF_ERROR_HOME_CHK_VAL);
			}
			break;
		case 3:	// 
...
// 현재 홈위치에 있는경우 - 반대로 동작하면서 홈센싱 벗어나는 곳까지 이동 
				...
				check_time_cnt = System_ticks; // #if HOME_FIND_TIME_CHECK_Z	// 일정시간이후 homeChkEnd 안되면 에러추가
				subMenu_ihpz = 7;
			}
			else {
		...
		case 3:
			...
			else { // #if HOME_FIND_TIME_CHECK_Z	// 일정시간이후 homeChkEnd 안되면 에러추가
				if((System_ticks - check_time_cnt) >  HOME_FINE_CHECK_MAX_TIME_Z) {
// Z-H 센서 에러 이거나 안 움직이면 기구물이 고장인 경우가 대부분 일 듯 			
...
//#if HOME_CHECK_STEP_CNT_REDUCE
				...
				check_time_cnt = System_ticks; // #if HOME_FIND_TIME_CHECK_Y	// 일정시간이후 homeChkEnd 안되면 에러추가
				subMenu_ihpy = 9;				
			}
			else { // 홈 찾기 
...
//dfpErr("devState.btMotorState[Y]=x[%x] y[%x] z[%x]\n",devState.btMotorState[MOTOR_X],devState.btMotorState[MOTOR_Y],devState.btMotorState[MOTOR_Z]);				
			...
			else { // #if HOME_FIND_TIME_CHECK_Y	// 일정시간이후 homeChkEnd 안되면 에러추가
