...
#define CMD_STOP 					0x10 // 16  - 사용안함.
#define CMD_BREAK					0x20 // 32
#define CMD_INFINITE			0x40 // 64 - 사용안함.
...
CMD_ENCODER_CLR_CNT_ENABLE(MOTOR_X); \
		if(encoderCountCheck(encoderCount,readCount) > ERR_ENCODER_COUNT_X) {	errorEncoder(MOTOR_X); } \
	}
#endif
...
CMD_ENCODER_CLR_CNT_ENABLE(MOTOR_Y); \
		if(encoderCountCheck(encoderCount,readCount) > ERR_ENCODER_COUNT_Y) {	errorEncoder(MOTOR_Y); } \
	}
#endif
...
dfp("======  Z  readCount=[%d] [%d]\n", readCount , encoderCount); \
CMD_ENCODER_CLR_CNT_ENABLE(MOTOR_Z); \
		if(encoderCountCheck(encoderCount,readCount) > ERR_ENCODER_COUNT_Z) {	errorEncoder(MOTOR_Z); } \				
	}
#endif
...
#define COORD_Z_TOP						0		
	...
	#define COORD_Z_WASH_TOP			(3.5 + COORD_Z_NEEDLE_GUIDE_GAP)		// 실측데이터(3.75)        wash port의 외측 맨위
...
#if CHROZEN_Z_HEIGHT_SENSOR_MODIFY
	#define VIAL_DETECT_DEPTH			5.5
#else
