...
// 20191228 - ethernet config전송을 위해서 추가
#define PACKCODE_LCAS_ETHERNET_CONFIG		(PACKET_DEVICE_ID_YLAS + 0x300)


// 20210910 - 서비스프로그램 
#if STORE_INFORMATION_ENVM
#define PACKCODE_LCAS_INFORMATION				(PACKET_DEVICE_ID_YLAS + 0x900)
#endif

#if RS422_ENABLE
#if KCJ_TCP_INTR
#define TCP_WRITE_PACKET	{ \
	if(devState.rs422if_connected) { RS422_ENQUEUE_PACKET; } \
	else { \
		signed char retERR; \
...
#define ACK_LCAS_PACKET(ReqHeader)	{ \
	...
	#define SEND_ACK_LCAS_PACKET(ReqHeader) { \
		if(devState.rs422if_connected == NO) { \
