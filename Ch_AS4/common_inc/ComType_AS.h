...
/*
	[3:0]   - state : select motor
	[6:4]   - home0 : 1(Home) / 3(Encoder) / 2(Needle)
	[7]     - X 
	[17:8]  - Count : select motor
	[19:18] - X
	[23:20] - State0
	[27:24] - State1
	[30:28] - home1 : 1(Home) / 3(Encoder) / 2(X)
	[31]    - X
*/
...
//	unsigned char sensorError;
	...
	unsigned char envmDataAvailable;
	
	unsigned int rs422if_connected; //
	unsigned int rs422if_packet;	//

	unsigned int rs422if_request;	//	연결되어있어서 요청이 들어오고 있는상태이다.
#if RS422_REQUEST_CMD_INDEX_ADD
	unsigned int rs422if_cmd_index; //	수신받은 packet의 누적카운트(Master는 연결되면 1부터 시작함,)
#endif

	unsigned int chk_Dir[STEP_MOTOR_COUNT];
	unsigned int moving[STEP_MOTOR_COUNT];
...
unsigned int maxSpeed[STEP_MOTOR_COUNT]; //
	...
	unsigned int cmdChkDir_RESET_Addr[STEP_MOTOR_COUNT];
	unsigned int cmdChkDir_READY_Addr[STEP_MOTOR_COUNT];
	unsigned int cmdChkDir_START_Addr[STEP_MOTOR_COUNT];
	unsigned int cmdChkDir_ACT_END_Addr[STEP_MOTOR_COUNT];

	unsigned int read_ChkDir_Addr[STEP_MOTOR_COUNT];
