...
#if ADJUST_VALUE_LIMIT_25
#else
#endif

#if ADJUST_VALUE_LIMIT_25
#define ADJUST_VALUE_MAX	25
#define ADJUST_VALUE_LIMIT	24
#define ADJUST_VALUE__NEEDLE_H_MAX 		25
#define ADJUST_VALUE__NEEDLE_H_LIMIT 	24

#define	ADJUST_VALUE_X_MIN		-ADJUST_VALUE_MAX
#define	ADJUST_VALUE_X_MAX		ADJUST_VALUE_MAX
#define	ADJUST_VALUE_Y_MIN		-ADJUST_VALUE_MAX
#define	ADJUST_VALUE_Y_MAX		ADJUST_VALUE_MAX

#define	ADJUST_VALUE_Z_MIN		-ADJUST_VALUE_MAX
#define	ADJUST_VALUE_Z_MAX		ADJUST_VALUE_MAX

#define	ADJUST_VALUE_SYR_MIN		-ADJUST_VALUE_MAX
#define	ADJUST_VALUE_SYR_MAX		ADJUST_VALUE_MAX

#define	ADJUST_VALUE_VAL_MIN		-ADJUST_VALUE_MAX
#define	ADJUST_VALUE_VAL_MAX		ADJUST_VALUE_MAX
#define	ADJUST_VALUE_INJ_MIN		-ADJUST_VALUE_MAX
#define	ADJUST_VALUE_INJ_MAX		ADJUST_VALUE_MAX
#else
#define ADJUST_VALUE_MAX	99
#define ADJUST_VALUE_LIMIT	98
#define ADJUST_VALUE__NEEDLE_H_MAX 		99
#define ADJUST_VALUE__NEEDLE_H_LIMIT 	98
...
#if RS422_MIX_STRUCT_DOWNSIZE
	#define MIX_TABLE_COUNT		10		// alias - 15step
#else
...
#define COMMAND_END					2		// Clrity의 정상적인      분석종료일 때만 전송된다. - led 전환에 사용 
#else
...
#define SELF_ERROR_CHECK_ENCODER_X				101
#define SELF_ERROR_CHECK_ENCODER_Y				102
#define SELF_ERROR_CHECK_ENCODER_Z				103
#define SELF_ERROR_CHECK_ENCODER_SYR			104
#define SELF_ERROR_CHECK_ENCODER_VAL			105
#define SELF_ERROR_CHECK_ENCODER_INJ			106


/*
#define SELF_ERROR_HOME_FIND_TIME_OUT_X			111
#define SELF_ERROR_HOME_FIND_TIME_OUT_Y			112
#define SELF_ERROR_HOME_FIND_TIME_OUT_Z			113
#define SELF_ERROR_HOME_FIND_TIME_OUT_SYR			114
#define SELF_ERROR_HOME_FIND_TIME_OUT_VAL			115
#define SELF_ERROR_HOME_FIND_TIME_OUT_INJ			116
*/
...
#define SELF_COMMAND_REJECT		7		// 전송 명령 거부 ,전송된 명령을 실행할 수 없는 상태일 전송됨.
...
// STORAGE_SYS_INFO_T 와 
// 저장시점은
// - 최초부팅후 
// UsedENVM 영역을 읽어서 검사 다를 경우 : 0xAA5555AA
// EEPROM Data eNVM으로 변환 저장 
// 1. 현재버전과 다를 경우
// 2. 시리얼 번호 입력 시점 
// 
typedef struct _CHROZEN_INFORMATION_t
{
	unsigned int	BLVer;	// Major(2) Minor(2)
	unsigned int	IAPVer;

//	unsigned int	APPVer;	//	
	unsigned int FirmwareVersion;	// RD 06/11 	버전	 "6.11" 	//	unsigned char cMachineVersion[2]; 	// RD 1/3 		버전	 "1.3"
//	unsigned char cFirmwareVersion[2];	// RD 01/01/03	버전	 "1.1.3"
//	unsigned char cMachineVersion[2]; 	// RD 01/03/08	버전	 "1.3.8"
	unsigned int	FPGAVer;
	
	unsigned char	cSerialNO[32]; //SerialNo[32];
	unsigned char	cIPAddress[4]; //IPAddress[4];
	unsigned char	cGateway[4];	//Gateway[4];
	unsigned char	cMacAddress[6];	//MacAddress[6];

	char cModel[12];	// cModel[12]		// RD	SYSTEM_MODEL		"YI9650"		-- AS / OVEN  연결시 검사 
// 74byte

	unsigned char blank[2];

// ===============================================================
	unsigned char cInstallDate[4];	// ex)	20/21/09/01		--- (bit 0)
	unsigned char cCurDate[4];			// ex)	20/21/09/01		--- (bit 1)
	unsigned char cCurTime[4];			// ex)	--/14/30/35		--- (bit 1)

// Oven / AS
	unsigned int setFlag;	// bit가 1이면 해당 항목 저장함.

	float fMaxTemperature;				// 최대설정 온도
	float fMinTemperature;				// 최소설정 온도

	unsigned char LED_onoff;		// AS									--- (bit 5)
	unsigned char Door_check;		// AS / Oven					--- (bit 5)
	unsigned char Leak_check;		// AS / Oven / UVD		--- (bit 5)
	unsigned char Buzzer_onoff;	// AS / Oven / UVD		--- (bit 5)

// ===============================================================
// 아랫것들은 Oven에서 안쓰는 걸로 
// OVEN_TEMPCAL_t 사용 
	unsigned char set_dummy1;			// (1)35 / (2)60		--- (bit 7)
	unsigned char set_dummy2;			// (1)35 / (2)60		--- (bit 7)
	unsigned char TempConModule;	// trayCooling - AS온도제어모듈 유무	--- (bit 6) 
	unsigned char setTemp;			// (1)35 / (2)60		--- (bit 7)
	float fCalTemp[2];					// 									--- (bit 7)

	unsigned char cNetmask[4];		// ex) "255.255.255.0"	--- (bit 3) 	// 고정 
	unsigned int PortNo;					// ex) "4242" 					--- (bit 3) 	// 고정

#if 0	// 20220720
	unsigned char cdummy1[4];		//  			--- (bit 8)
	unsigned char cdummy2[4];		//  			--- (bit 9)
#else
// eNVM 저장 이전 버전의 데이터를 복구하기위한 플래그
	unsigned int 	UsedENVM;	// STORAGE_LAMP_INFO_PKT : FRAM -> ENVM 복사 완료 or
								// STORAGE_WAVE_INFO_PKT :FRAM -> ENVM 사용 변경.
#endif								
} INFORMATION_t;// CHROZEN_INFORMATION_t;
