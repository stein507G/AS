...
void ShowMemory(unsigned char Header, unsigned char * data, unsigned int Length)
{
	...
	{
		iprintf("----------------- Display Memory --------------------\r\n");
		iprintf("NUM : 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F \r\n");
		PrintLine();

		for(i=0;i<Length;i++)
		{
			if((i%0x10)==0) iprintf("%03X : ",(i>>4));
			iprintf("%02X,",data[i]);
			if((i%0x10)==0xF) iprintf("\r\n");
		}
		iprintf("\r\n");PrintLine();iprintf("\r\n");
	}
...
//	ver = FPGA_READ_WORD(0);
	...
	if((ver&0x11000000) == 0x11000000) {
		printf("          Fabric Ver (Rs422 Slave) : [%lX] \r\n", ver);		
//		return 3;
		return 1;
	}
	else if(ver&0x01000000)
	{
		printf("          Fabric Ver (Slave)       : [%lX] \r\n", ver);		
		return 1;
	}
	else
	{
		printf("          Fabric Ver (Master)      : [%lX] \r\n", ver);		
		return 0;
	}
}
...
//	dp0("               Application (eNVM) \r\n");
  ...
  iprintf("          Build Time : ["__DATE__"] ["__TIME__"]\n");		
	iprintf("-------------------------------------------------\r\n");
	iprintf("                      Firmware Ver :    [" FIRMWARE_VERSION "] \r\n");
