...
//#if CHECK_AS_INIT_POSITION_ENCODER_TEST
extern uint8_t subMenu_aipec;
...
#if MOVE_STEP_SPEED_ERROR_2_STEP_FIX
#define TOTAL_STEP_LIMIT	2
#else
#define TOTAL_STEP_LIMIT	0
#endif
...
//#if MOVE_STEP_SPEED_ERROR_2_STEP_FIX
	if(tStep > TOTAL_STEP_LIMIT) {	// 반대홈
	...
	else if(tStep < -TOTAL_STEP_LIMIT) {
		totalStep = -tStep;
		if(devSet.homeDir[who]== DIR_CCW) {// MOTOR_X , MOTOR_Y , MOTOR_SYR
			dir = DIR_CCW;
			ctrlCmd = DIR_CCW | CMD_START;
 	...
 	totalHaif = totalStep >> 1;	// 	totalHaif = totalStep / 2;
...
//	int totalHaif = totalStep / 2;
	uint32_t totalHaif = totalStep / 2;
...
//dfp("[%d] total=%d  maxSpeed=%d Accel=%d Deacc=%d ctrlCmd=0x%x\n",who,totalStep, maxSpeed, AccelStep, DeaccStep, ctrlCmd);
			*((uint32_t volatile *)(baseAddr + TOTAL_OFFSET)) = totalStep - 1;		// 하나 작은 값으로 저장한다(전송). FPGA에서 쉽게 사용하도록
			*((uint32_t volatile *)(baseAddr + START_SPEED)) = minSpeed;		// start_speed : 이값은 배열의 위치이므로 그대로 전송
			*((uint32_t volatile *)(baseAddr + ACCEL_OFFSET)) = AccelStep - 1;		// 하나 작은 값으로 저장한다(전송). FPGA에서 쉽게 사용하도록
			*((uint32_t volatile *)(baseAddr + DEACC_OFFSET)) = DeaccStep - 1;		// 하나 작은 값으로 저장한다(전송). FPGA에서 쉽게 사용하도록
...
int goRetryPos_X(int32_t pos, uint32_t option)
{
	int32_t	readCount=0;
...
//			Vial.uPos.tray = Tray;
			...
			encoderCount = goX_Encoder(&Vial, option);
#else
...
int goRetryPos_Y(int32_t pos, uint32_t option)
{
	int32_t	readCount=0;
	static int encoderCount = 0;
...
#if VIAL_XY_CALIBRATION_FIX_Y
			VIAL_POS_t Vial;
			Vial.pos = pos;
			
			encoderCount = goY_Encoder(&Vial, option);
dfp("encoderCount =%d\n",encoderCount);
#else
...
DEVSTATE_MOVING(MOTOR_X,OFF);
			...
			break;
		case 2:	// 홈부터 다시 찾고 
			if(goRetryPos_X(devState.mActSet[MOTOR_X].pos,   devState.mActSet[MOTOR_X].option)) {
				++subMenu_rcrx;
			}
			break;
		case 3:
...
DEVSTATE_MOVING(MOTOR_Y,OFF);				
			...
			break;
		case 2:
			if(goRetryPos_Y(devState.mActSet[MOTOR_Y].pos, devState.mActSet[MOTOR_Y].option)) {
				++subMenu_rcry;
			}
			break;
		case 3:			
...
int goPosVial(const VIAL_POS_t *pVial)
{
	...
	switch(subMenu_gpv) {
		case 1:
#if VIAL_XY_CALIBRATION_FIX_Y
			encoderCount_Y = goY_Encoder(pVial, ACT_XY_POS);
#else
		...
		case 3:
#if VIAL_XY_CALIBRATION_FIX
			encoderCount_X = goX_Encoder(pVial, ACT_XY_POS);
#else
...
#if VIAL_XY_CALIBRATION_FIX
int goX_Encoder(const VIAL_POS_t *pVial, uint32_t option)
{
	int32_t step;
	int32_t cur,act;
	int ret = 0;
	...
	CMD_ENCODER_CLR_CNT_ENABLE(MOTOR_X);
//	devState.mActSet[MOTOR_X].pos = pVial->uPos.x; //pos;	// vial 좌표 a~e (a1 .. e12)
///	devState.mActSet[MOTOR_X].Tray = pVial->uPos.tray; //Tray;
	...
	switch(option) {
		case ACT_XY_POS:
			switch(pVial->uPos.tray) {
				case TRAY_LEFT:
					act = xyzCoord.vialStep_L_X[pVial->uPos.x] + devSet.adjust.leftVial_x[sysConfig.trayLeft] * pVial->uPos.y / devSet.trayLeft_colNo;
					devState.curStep[MOTOR_X] = act;
					devState.curTray = pVial->uPos.tray;//Tray;
					break;
				case TRAY_RIGHT:
					act = xyzCoord.vialStep_R_X[pVial->uPos.x] + devSet.adjust.rightVial_x[sysConfig.trayRight] * pVial->uPos.y / devSet.trayRight_colNo;
					devState.curStep[MOTOR_X] = act;
					devState.curTray = pVial->uPos.tray;//Tray;
					break;
				case TRAY_ETC:
					act = xyzCoord.etcStep_X[pVial->uPos.etc];
					devState.curStep[MOTOR_X] = act;
					devState.curTray = pVial->uPos.tray;//Tray;
					break;
			}
	...
	devState.mActSet[MOTOR_X].pos = pVial->pos;
...
#if CHECK_ENCODER_DIRECTION
		if(ret == MOVE_CMD_OK) {
			CMD_CHK_DIR_START(MOTOR_X);
//			devState.moving[MOTOR_X] = ON;
	//	devState.chk_Dir[MOTOR_X] = READ_CHK_DIR(MOTOR_X);
	//	dprintf("## CMD_CHK_DIR_START chk_DirX=%08x cur=%d act=%d step=%d\n",devState.chk_Dir[MOTOR_X], cur, act, step);
		}		
#endif
...
//else {	
//devState.curStep[MOTOR_X] = desStep;
//}
	return encoderCount;
...
//					devState.curTray = Tray;
 ...
 #if VIAL_XY_CALIBRATION_FIX_Y
int goY_Encoder(const VIAL_POS_t *pVial, uint32_t option) 
{
	int32_t step;
	int32_t cur,act;
	int ret = 0;
...
///	devState.mActSet[MOTOR_Y].Tray = pVial->uPos.tray; //Tray;
	...
	switch(option) {
		case ACT_XY_POS:
			switch(pVial->uPos.tray) {
				case TRAY_LEFT:
					act = xyzCoord.vialStep_L_Y[pVial->uPos.y];
					act = act + devSet.adjust.leftVial_y[sysConfig.trayLeft] * pVial->uPos.x / devSet.trayLeft_rowNo;
					devState.curStep[MOTOR_Y] = act;
					break;
				case TRAY_RIGHT:
					act = xyzCoord.vialStep_R_Y[pVial->uPos.y];		
					act = act + devSet.adjust.rightVial_y[sysConfig.trayRight] * pVial->uPos.x / devSet.trayRight_rowNo;				
					devState.curStep[MOTOR_Y] = act;
					break;
				case TRAY_ETC:
					act = xyzCoord.etcStep_Y[pVial->uPos.etc];
					devState.curStep[MOTOR_Y] = act;
					break;
			}
			break;
	...
	devState.mActSet[MOTOR_Y].pos = pVial->pos;
...
#if CHECK_ENCODER_DIRECTION
		if(ret == MOVE_CMD_OK) {
			CMD_CHK_DIR_START(MOTOR_Y);
	//	devState.chk_Dir[MOTOR_Y] = READ_CHK_DIR(MOTOR_Y);
	//	dprintf("## CMD_CHK_DIR_START chk_DirY=%08x cur=%d act=%d step=%d\n",devState.chk_Dir[MOTOR_Y], cur, act, step);
		}
#endif
...
#if CHECK_ENCODER_DIRECTION
		if(ret == MOVE_CMD_OK) {
			CMD_CHK_DIR_START(MOTOR_Z);
	//	devState.chk_Dir[MOTOR_Z] = READ_CHK_DIR(MOTOR_Z);
//		dprintf("## CMD_CHK_DIR_START chk_DirZ=%08x cur=%d act=%d step=%d\n",devState.chk_Dir[MOTOR_Z], cur, act, step);
		}
#endif
...
int goPosETC_Z(int32_t xy_pos, int32_t z_pos)//, uint32_t z_option)
{
...
#if VIAL_XY_CALIBRATION_FIX
	VIAL_POS_t Vial;
	Vial.uPos.etc = xy_pos;
	Vial.uPos.tray = TRAY_ETC;
#endif
	...
	switch(subMenu_gpez) {
		...
		case 4:
#if VIAL_XY_CALIBRATION_FIX
			encoderCount_X = goX_Encoder(&Vial, ACT_XY_POS);
#else
		...
		case 5:
#if VIAL_XY_CALIBRATION_FIX_Y			
			encoderCount_Y = goY_Encoder(&Vial, ACT_XY_POS);
#else
...
int goPosETC_Z_check(int32_t xy_pos, int32_t z_pos, int32_t z_chk_pos, int32_t z_chk_upper_pos)
{
	...
	switch(subMenu_gpezc) {
		case 1:	// 현재위치가 목표위치면 
			...
			// POS_Z_TOP은 센서 검출 상태
				encoderCount = goZ_Encoder(POS_Z_TOP,ACT_Z_POS);	// CMD_CHK_DIR_START_Z
				subMenu_gpezc++;
			}
			break;
		case 2:
...
//						readMotorState();
	dfpErr("===devState.btMotorState[MOTOR_Z] = [%x]\n",devState.btMotorState[MOTOR_Z]);
	...
	// 하강후 포토센서 체크에러 난 경우임 - 높이 조정해야 될 것을 판단됨 - 아닌가 ????
			...
			break;
		case 8:
//	dfpErr("WAIT_TIME_VALUE_CASE_STATE devState.btMotorState[MOTOR_Z] = [%x]\n",devState.btMotorState[MOTOR_Z]);			
			WAIT_TIME_VALUE_CASE_STATE(subMenu_gpezc, DELAY_Z_ERROR_RECHECK_TIME);
			if((devState.btMotorState[MOTOR_Z] & 0x30) ==	 0x00) {		// 2번		
				encoderCount = goZ_Encoder(z_pos, ACT_Z_POS);
				subMenu_gpezc = 10;
// 20211222			
	dfpErr("WAIT_TIME_VALUE_CASE_STATE = PASS PASS [MOTOR_Z] = [%x]\n",devState.btMotorState[MOTOR_Z]);
			}			
			break;
		...
		case 9:	// 대기후 다시 검사
	dfpErr("===XXX9999 devState.btMotorState[MOTOR_Z] = [%x] [%x]\n",devState.btMotorState[MOTOR_Z],devState.btMotorState[MOTOR_X]);
			if(readyChkRetry_Z(encoderCount)) {
	dfpErr("===XXX9999------\n");
				if((devState.btMotorState[MOTOR_Z] & 0x30) == 0x00) {			// 3번째   검사
	dfpErr("===XXX9999----------- OK\n");
					encoderCount = goZ_Encoder(z_pos, ACT_Z_POS);
					actRetry_gpezc = 1;
					subMenu_gpezc++;
	...
	dfpErr("===Z err -------------- devState.btMotorState[MOTOR_Z] = [%x] [%x]\n",devState.btMotorState[MOTOR_Z],devState.btMotorState[MOTOR_X]);
	      	if(actRetry_gpezc == 1) {	// Top으로 이동후 다시한번 시도
	      		actRetry_gpezc++;
	dfpErr("===actRetry_gpezc=%d \n",actRetry_gpezc);
	      		subMenu_gpezc = 11;
	        }
	      	else { // 에러 처리 - 메시지 전송후 상태는 state_fault로
	      		actRetry_gpezc = 1;
...
#if CHECK_ENCODER_DIRECTION
		if(ret == MOVE_CMD_OK) {
			CMD_CHK_DIR_START(MOTOR_SYR);
		devState.chk_Dir[MOTOR_SYR] = READ_CHK_DIR(MOTOR_SYR);
		dprintf("## MOTOR_SYR CMD_CHK_DIR_START chk_DirS=%08x cur=%f act=%f step=%d\n",devState.chk_Dir[MOTOR_SYR], cur, act, step);
		}
#endif
...
int initHomePos_Val()	// Valve만 따로 홈위치 찾기 
{
	...
	static uint32_t check_time_cnt = 0;

	if(System_ticks > check_time_cnt && subMenu_ihpv > 1) { // 시간 초과 
		errorFunction(SELF_ERROR_HOME_CHK_VAL);
	}
	...
	switch(subMenu_ihpv) {
		case 1:
			...
			++subMenu_ihpv;
			check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_VAL; // #if HOME_FIND_TIME_CHECK_VAL	// 일정시간이후 homeChkEnd 안되면 에러추가			
			break;
		case 2:
...
#if MOVE_STEP_SPEED_FUNCTION_CHANGE
					if(devSet.adjust.value[MOTOR_VAL] > 0) stepControl(MOTOR_VAL, R_HOME_DIR_VAL | CMD_START, devSet.chkHomeSpeed[MOTOR_VAL], devSet.adjust.value[MOTOR_VAL] * ADJUST_STEP_UNIT_VAL);
					else if(devSet.adjust.value[MOTOR_VAL] < 0) stepControl(MOTOR_VAL, HOME_DIR_VAL | CMD_START, devSet.chkHomeSpeed[MOTOR_VAL], -devSet.adjust.value[MOTOR_VAL] * ADJUST_STEP_UNIT_VAL);
#else
...
int initHomePos_Inj()	// 인젝터만 따로 홈위치 찾기
{
	...
	if(System_ticks > check_time_cnt && subMenu_ihpi > 1) { // 시간 초과 
		errorFunction(SELF_ERROR_HOME_CHK_INJ);
	}
	...
	switch(subMenu_ihpi) {
		case 1:
			...
			++subMenu_ihpi;
			check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_INJ; // #if HOME_FIND_TIME_CHECK_INJ	// 일정시간이후 homeChkEnd 안되면 에러추가						
			break;
		case 2:
		...
		case 12:	// 홈찾은후 adjust(조정)값 만큼 이동 한다.  -끝
			if(readyChkVal(MOTOR_INJ)) {	// RESET이후 다시 READY상태 --> 다시 홈위치 찾는 동작 --> 1.8도 단위로 (8pulse)
				...
				if((devState.btMotorState[MOTOR_INJ] & INJ_HOME1_CHECK) == INJ_HOME1_SENSING ) {
					...
					// 막힌 부분 - STEP_MORE_HOME_INJ(8pulse) 만 돌고 멈춘다. -끝
#if MOVE_STEP_SPEED_FUNCTION_CHANGE
					if(devSet.adjust.value[MOTOR_INJ] > 0) stepControl(MOTOR_INJ, R_HOME_DIR_INJ | CMD_START, devSet.chkHomeSpeed[MOTOR_INJ], devSet.adjust.value[MOTOR_INJ] * ADJUST_STEP_UNIT_VAL);
					else if(devSet.adjust.value[MOTOR_INJ] < 0) stepControl(MOTOR_INJ, HOME_DIR_INJ | CMD_START, devSet.chkHomeSpeed[MOTOR_INJ], -devSet.adjust.value[MOTOR_INJ] * ADJUST_STEP_UNIT_VAL);
#else
...
int initHomePos_Syr()	// 
{
	...
	if(System_ticks > check_time_cnt && subMenu_ihps > 1) { // 시간 초과 
	...
	switch(subMenu_ihps) {
		case 1:
			...
			{	// READY 상태검사 
				...
				check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_INJ; // #if HOME_FIND_TIME_CHECK_INJ	// 일정시간이후 homeChkEnd 안되면 에러추가										
#if CHECK_ENCODER_DIRECTION
CMD_CHK_DIR_RESET(MOTOR_SYR);
// Syringe Motor는 엔코더가 반대로 장착되어 있다. 출력되는 값이 반대임. - FPGA에서 옵션사항으로 추가해야 함. 
CMD_CHK_DIR_OPTION_DIR_SYR = 0x01; 	// cur_DIR출력 반전
#endif
			}
			break;
		case 2:
		...
		case 3:	// 홈위치 찾기
			if(readyChk(MOTOR_SYR)) {
				if((devState.btMotorState[MOTOR_SYR] & SYR_HOME1_CHECK) == SYR_HOME1_SENSING) {	
					errorFunction(SELF_ERROR_HOME_CHK_SYR);					// error
				}
				else {
...
if(subMenu_ihpz != Menu) {
	...
	if(System_ticks > check_time_cnt && subMenu_ihpz > 1) { // 시간 초과 
		errorFunction(SELF_ERROR_HOME_CHK_Z);
	}
	...
	switch(subMenu_ihpz) {
		case 1:
			...
			{	// READY 상태검사 
				++subMenu_ihpz;
				homeChk_retryCnt = 0;
				check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_Z;
#if CHECK_ENCODER_DIRECTION
			CMD_CHK_DIR_RESET(MOTOR_Z);
			CMD_CHK_DIR_OPTION_DIR_Z = 0x00;	// cur_DIR출력
#endif
			}
			break;
		case 2:
...
#if HOME_CHECK_STEP_CNT_REDUCE
				stepControl(MOTOR_Z, R_HOME_DIR_Z | CMD_START, devSet.chkHomeSpeed[MOTOR_Z], 3200);	// 4.88mm
#else
...
uint32_t stepCount;
	...
	if(System_ticks > check_time_cnt && subMenu_ihpx > 1) { // 시간 초과 
//stepCount = READ_STEP_COUNT(MOTOR_X);
//dfp("stepCount_3=%d\n",stepCount);
	...
	switch(subMenu_ihpx) {
		case 1:
			...
			{	// READY 상태검사 
				++subMenu_ihpx;
				homeChk_retryCnt = 0;
				check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_X; // #if HOME_FIND_TIME_CHECK_X
#if CHECK_ENCODER_DIRECTION
CMD_CHK_DIR_RESET(MOTOR_X);
CMD_CHK_DIR_OPTION_DIR_X = 0x00;	// cur_DIR출력
#endif

CMD_CLR_STEP_COUNT(MOTOR_X);
CMD_EN_STEP_COUNT(MOTOR_X);
			}
			break;
		case 2:
			if((devState.btMotorState[MOTOR_X] & X_HOME1_CHECK) == X_HOME1_SENSING) {	// 현재 홈위치에 있는경우 - 반대로 동작후 다시 센싱
				stepControl(MOTOR_X, R_HOME_DIR_X | CMD_START, devSet.chkHomeSpeed[MOTOR_X], 1600); 
...
CMD_EN_STEP_COUNT(MOTOR_X);
		...
		case 8:
			stepControl(MOTOR_X, R_HOME_DIR_X | CMD_START, devSet.chkHomeSpeed[MOTOR_X], 1600); 
//			stepControl(MOTOR_X, R_HOME_DIR_X | CMD_START, devSet.chkHomeSpeed[MOTOR_X], 32); 			
			devSet.ucChkHome_R[MOTOR_X] = YES;
			subMenu_ihpx++;				
			break;
 		case 9:
...
#if CHECK_ENCODER_DIRECTION
CMD_CHK_DIR_RESET(MOTOR_X);
CMD_CHK_DIR_OPTION_DIR_X = 0x00;	// cur_DIR출력설정
//dprintf("##CMD_CHK_DIR_RESET(MOTOR_X)=%08x\n",devState.chk_Dir[MOTOR_X]);
#endif
...
//CMD_EN_STEP_COUNT(MOTOR_X);
		...
		case 11:
			devState.curTray = TRAY_ETC;
			devState.curStep[MOTOR_X] = 168;	// 	156 + 8  --- 원래는 160(홈찾은후 이동) + 8(slit);
...
if(subMenu_ihpy != Menu) {
	...
	if(System_ticks > check_time_cnt && subMenu_ihpy > 1) { // 시간 초과 
	...
	switch(subMenu_ihpy) {
		case 1:
			...
			{	// READY 상태검사 
				++subMenu_ihpy;
				homeChk_retryCnt = 0;
				check_time_cnt = System_ticks + HOME_FINE_CHECK_MAX_TIME_Y;
#if CHECK_ENCODER_DIRECTION
CMD_CHK_DIR_RESET(MOTOR_Y);
CMD_CHK_DIR_OPTION_DIR_Y = 0x00;	// cur_DIR출력
#endif
			}
			break;
		case 2:
			if((devState.btMotorState[MOTOR_Y] & Y_HOME1_CHECK) == Y_HOME1_SENSING) {	// 현재 홈위치에 있는경우 - 반대로 동작후 다시 센싱
				stepControl(MOTOR_Y, R_HOME_DIR_Y | CMD_START, devSet.chkHomeSpeed[MOTOR_Y], 1600);
...
if(subMenu_ihpy != Menu) {
	...
	switch(subMenu_ihpy) {
		case 1:
			...
			CMD_CHK_DIR_RESET(MOTOR_Y);
			CMD_CHK_DIR_OPTION_DIR_Y = 0x00;	// cur_DIR출력
//dprintf("##CMD_CHK_DIR_RESET(MOTOR_Y)=%08x\n",devState.chk_Dir[MOTOR_Y]);
#endif
			}
			break;
		case 2:
...
#if 1
int AS_INIT_Position_Encoder_Check()
{
	int32_t	readCount=0;
	static int encoderCount_X = 0;
	static int encoderCount_Y = 0;

	static unsigned int minSpeed[6];
	static unsigned int maxSpeed[6];


	static VIAL_POS_t Vial;

#if DEBUG_MOTORCTRL_SUBMENU_NO
	static uint32_t Menu = 0;
	if(subMenu_aipec != Menu) {
		dfp("subMenu_aipec=%d\n",subMenu_aipec);
		Menu = subMenu_aipec;
	}
#endif
	
	switch(subMenu_aipec) {
	case 1:
//		if(sleepChk(MOTOR_X)) { motorReadyCmd(MOTOR_X);
			Vial.uPos.tray = TRAY_LEFT;
			Vial.uPos.x = 0;
			Vial.uPos.y = 0;
			Vial.uPos.etc = POS_XY_INJECTOR;			

			for(int i=0; i<6 ; i++) {
				minSpeed[i] = devSet.minSpeed[i];
//				devSet.minSpeed[i] = 0;
				maxSpeed[i] = devSet.maxSpeed[i];
//				devSet.maxSpeed[i] = 115;
			}
//			devSet.maxSpeed[0] = 130;	
//			devSet.maxSpeed[1] = 116;	// 95  
			// Y축-1 X 104 ~ 109  // 116 ~ 120
			// Y축-2 X 99 ~ 106	  // O 115 ~ 120

			// X축-1 X 94(소음) ~ 99  128 ~ 
			// X축-2 X 94(Err) ~ 99  116~128(Err - 멈춤 )  130 ~ 139는 이상무 이상하네 - 공진 발생하는 구간에서 토크가 확 떨어진다??
//		}
		++subMenu_aipec;
		break;
	case 2:
dprintf("2============MaxSpeed = %d %d\n",devSet.maxSpeed[0],devSet.maxSpeed[1]);
#if VIAL_XY_CALIBRATION_FIX_Y
		encoderCount_Y = goY_Encoder(&Vial, ACT_XY_POS);
#else
		encoderCount_Y = goY_Encoder(&Vial->uPos.y, &Vial->uPos.tray, ACT_XY_POS);
#endif
		subMenu_aipec++;
		break;
	case 3:
		WAIT_TIME_VALUE_CASE_STATE(subMenu_aipec, MOVE_XY_DELAY_TIME);
		break;
	case 4:
#if VIAL_XY_CALIBRATION_FIX
		encoderCount_X = goX_Encoder(&Vial, ACT_XY_POS);
#else
		encoderCount_X = goX_Encoder(&Vial->uPos.x, &Vial->uPos.tray, ACT_XY_POS);
#endif
		subMenu_aipec++;
		break;
	case 5:
	...
	case 7:
		...
		Vial.uPos.x = 0;
		Vial.uPos.y = 0;
		Vial.uPos.etc = POS_XY_HOME;

		subMenu_aipec++;		
		break;
	case 8:
dprintf("8============MaxSpeed = %d %d\n",devSet.maxSpeed[0],devSet.maxSpeed[1]);				
#if VIAL_XY_CALIBRATION_FIX_Y
		encoderCount_Y = goY_Encoder(&Vial, ACT_XY_POS);
#else
		encoderCount_Y = goY_Encoder(&Vial->uPos.y, &Vial->uPos.tray, ACT_XY_POS);
...
//		Vial.uPos.etc = POS_XY_HOME;			
		...
		for(int i=0; i<6 ; i++) {
			devSet.maxSpeed[i] += 1;//10;
//			devSet.maxSpeed[0] = 139;//145;
		}
		
dprintf("============MaxSpeed = %d %d\n",devSet.maxSpeed[0],devSet.maxSpeed[1]);
		if(devSet.maxSpeed[1] > 125) {	
			for(int i=0; i<6 ; i++) {
				devSet.minSpeed[i] = minSpeed[i];
				devSet.maxSpeed[i] = maxSpeed[i];
			}
dprintf("=========org MaxSpeed = %d %d\n",devSet.maxSpeed[0],devSet.maxSpeed[1]);
//			subMenu_aipec++;			
		subMenu_aipec = 1;
...
//				devSet.maxSpeed[i] = 115;
			...
			devSet.maxSpeed[0] = 114;	
			devSet.maxSpeed[1] = 100;	// 95  
			// Y축-1 X 104 ~ 109  // 116 ~ 120
			// Y축-2 X 99 ~ 106	  // O 115 ~ 120
...
#if INIT_COORDINATE_FIX		
	xyzCoord.etcStep_X[POS_XY_INJECTOR] 	= (int)(COORD_X_INJECTOR / MM_PER_STEP_X * MICRO_STEP_X + 0.5);
		xyzCoord.etcStep_X[POS_XY_INJECTOR] +=	devSet.adjust.value[ADJUST_INJ_PORT_VALUE_X] * ADJUST_STEP_UNIT_X;
		
	xyzCoord.etcStep_X[POS_XY_WASTE] 		= (int)(COORD_X_WASTE / MM_PER_STEP_X * MICRO_STEP_X + 0.5);
		xyzCoord.etcStep_X[POS_XY_WASTE] += devSet.adjust.waste_x * ADJUST_STEP_UNIT_X;

	xyzCoord.etcStep_X[POS_XY_WASH] 			= (int)(COORD_X_WASH / MM_PER_STEP_X * MICRO_STEP_X + 0.5);
		xyzCoord.etcStep_X[POS_XY_WASH] += devSet.adjust.wash_x * ADJUST_STEP_UNIT_X;
		
	xyzCoord.etcStep_X[POS_XY_EXCHANGE] 			= (int)(COORD_X_EXCHANGE / MM_PER_STEP_X * MICRO_STEP_X + 0.5);
		xyzCoord.etcStep_X[POS_XY_EXCHANGE] += devSet.adjust.value[ADJUST_INJ_PORT_VALUE_X] * ADJUST_STEP_UNIT_X;

	xyzCoord.etcStep_X[POS_XY_HOME] 			= (int)(COORD_X_HOME / MM_PER_STEP_X * MICRO_STEP_X + 0.5);
	
	xyzCoord.etcStep_Y[POS_XY_INJECTOR] 	= (int)(COORD_Y_INJECTOR / MM_PER_STEP_Y * MICRO_STEP_Y + 0.5);
		xyzCoord.etcStep_Y[POS_XY_INJECTOR] += devSet.adjust.value[ADJUST_INJ_PORT_VALUE_Y] * ADJUST_STEP_UNIT_Y;
		
	xyzCoord.etcStep_Y[POS_XY_WASTE] 		= (int)(COORD_Y_WASTE / MM_PER_STEP_Y * MICRO_STEP_Y + 0.5);
		xyzCoord.etcStep_Y[POS_XY_WASTE] += devSet.adjust.waste_y * ADJUST_STEP_UNIT_Y;	
		
	xyzCoord.etcStep_Y[POS_XY_WASH] 			= (int)(COORD_Y_WASH / MM_PER_STEP_Y * MICRO_STEP_Y + 0.5);
		xyzCoord.etcStep_Y[POS_XY_WASH] += devSet.adjust.wash_y * ADJUST_STEP_UNIT_Y;	

	xyzCoord.etcStep_Y[POS_XY_EXCHANGE] 	= (int)(COORD_Y_EXCHANGE / MM_PER_STEP_Y * MICRO_STEP_Y + 0.5);
		xyzCoord.etcStep_Y[POS_XY_EXCHANGE] += devSet.adjust.value[ADJUST_INJ_PORT_VALUE_Y] * ADJUST_STEP_UNIT_Y;
		
	xyzCoord.etcStep_Y[POS_XY_HOME] 		= (int)(COORD_Y_HOME / MM_PER_STEP_Y * MICRO_STEP_Y + 0.5);
#else
// MICRO_STEP_Y 곱하기 전에 반올림한다. - MICRO_STEP_Y 단위로 맞추기위해서
// 최대로 차이나는 Step수는 7step이다. 
// z축: 2.4384mm / 1600 * 7 = 0.010668mm , x축: 9.7536mm / 1600 * 7 = 0.042672mm 차이나므로 신경안써도 된다. 
...
//dprintf("xyzCoord.step_Z[POS_Z_WASH_TOP] = [%d] AdjustVialHeight_Z= [%d]\n",xyzCoord.step_Z[POS_Z_WASH_TOP], AdjustVialHeight_Z);
...
dprintf("xyzCoord.step_Z[POS_Z_INJECTION_PORT_DETECT] = [%d] portDetectDepthStep= [%d]\n",xyzCoord.step_Z[POS_Z_INJECTION_PORT_DETECT], portDetectDepthStep);	

#if Z_POS_CHECK_INJPORT_VALUE_MODIFY
// AdjustVialHeight_Z
	xyzCoord.step_Z[POS_Z_INJECTION_PORT_UPPER_DETECT] = (int)((COORD_Z_INJECTOR_TOP) / MM_PER_STEP_Z + 0.5) * (int)(MICRO_STEP_Z);
	...
	xyzCoord.step_Z[POS_Z_INJECTION_PORT_UPPER_DETECT] = (int)((COORD_Z_INJECTOR_TOP - 1) / MM_PER_STEP_Z + 0.5) * (int)(MICRO_STEP_Z);
		xyzCoord.step_Z[POS_Z_INJECTION_PORT_UPPER_DETECT] += AdjustVialHeight_Z;
#endif
dprintf("xyzCoord.step_Z[POS_Z_INJECTION_PORT_UPPER_DETECT] = [%d] AdjustVialHeight_Z= [%d]\n",xyzCoord.step_Z[POS_Z_INJECTION_PORT_UPPER_DETECT], AdjustVialHeight_Z);			

#if	Z_POS_CHECK_WASHPORT_VALUE_MODIFY
	portDetectDepthStep = (int)(2.5 / MM_PER_STEP_Z + 0.5) * (int)(MICRO_STEP_Z);	
	xyzCoord.step_Z[POS_Z_WASH_UPPER_DETECT] = xyzCoord.step_Z[POS_Z_WASH_TOP] + portDetectDepthStep; // 기존 보다 2.5mm 아래
	...
	portDetectDepthStep = (int)(2.5 / MM_PER_STEP_Z + 0.5) * (int)(MICRO_STEP_Z);	
	xyzCoord.step_Z[POS_Z_WASTE_UPPER_DETECT] = xyzCoord.step_Z[POS_Z_WASH_TOP] + portDetectDepthStep;
...
dprintf("xyzCoord.step_Z[POS_Z_WASH_DETECT] = [%d] xyzCoord.step_Z[POS_Z_WASTE_UPPER_DETECT] = [%d] portDetectDepthStep= [%d]\n",xyzCoord.step_Z[POS_Z_WASH_DETECT],xyzCoord.step_Z[POS_Z_WASTE_UPPER_DETECT], portDetectDepthStep);	
...
dprintf("================= AdjustVialHeight_x = %d vial_adjust_length_mm_x %f \n",  AdjustVialHeight_x , vial_adjust_length_mm_x);
dprintf("== %d %d atan()= %f \n",  AdjustVialHeight_x , AdjustVialHeight_y, atan((float)AdjustVialHeight_y / (float)AdjustVialHeight_x));

//x= asin(AdjustVialHeight_x / Y 트레이 길이(step)) 
...
#if CHECK_ENCODER_DIRECTION
	devSet.cmdChkDir_RESET_Addr[MOTOR_X] = BASEADDR_MOTOR_X_Z + (0x30C << 2);
	devSet.cmdChkDir_RESET_Addr[MOTOR_Y] = BASEADDR_MOTOR_Y_SYR + (0x308 << 2);
	devSet.cmdChkDir_RESET_Addr[MOTOR_Z] = BASEADDR_MOTOR_X_Z + (0x308 << 2);
	devSet.cmdChkDir_RESET_Addr[MOTOR_SYR] = BASEADDR_MOTOR_Y_SYR + (0x30C << 2);
	//devSet.cmdChkDir_RESET_Addr[MOTOR_VAL] = BASEADDR_MOTOR_X_Z + (0x30C << 2);
	//devSet.cmdChkDir_RESET_Addr[MOTOR_INJ] = BASEADDR_MOTOR_X_Z + (0x30C << 2);

	devSet.cmdChkDir_READY_Addr[MOTOR_X] = BASEADDR_MOTOR_X_Z + (0x30D << 2);
	devSet.cmdChkDir_READY_Addr[MOTOR_Y] = BASEADDR_MOTOR_Y_SYR + (0x309 << 2);
	devSet.cmdChkDir_READY_Addr[MOTOR_Z] = BASEADDR_MOTOR_X_Z + (0x309 << 2);
	devSet.cmdChkDir_READY_Addr[MOTOR_SYR] = BASEADDR_MOTOR_Y_SYR + (0x30D << 2);
	//devSet.cmdChkDir_READY_Addr[MOTOR_VAL] = BASEADDR_MOTOR_X_Z + (0x30D << 2);
	//devSet.cmdChkDir_READY_Addr[MOTOR_INJ] = BASEADDR_MOTOR_X_Z + (0x30D << 2);

	devSet.cmdChkDir_START_Addr[MOTOR_X] = BASEADDR_MOTOR_X_Z + (0x30E << 2);
	devSet.cmdChkDir_START_Addr[MOTOR_Y] = BASEADDR_MOTOR_Y_SYR + (0x30A << 2);
	devSet.cmdChkDir_START_Addr[MOTOR_Z] = BASEADDR_MOTOR_X_Z + (0x30A << 2);
	devSet.cmdChkDir_START_Addr[MOTOR_SYR] = BASEADDR_MOTOR_Y_SYR + (0x30E << 2);
	//devSet.cmdChkDir_START_Addr[MOTOR_VAL] = BASEADDR_MOTOR_X_Z + (0x30E << 2);
	//devSet.cmdChkDir_START_Addr[MOTOR_INJ] = BASEADDR_MOTOR_X_Z + (0x30E << 2);

	devSet.cmdChkDir_ACT_END_Addr[MOTOR_X] = BASEADDR_MOTOR_X_Z + (0x30F << 2);
	devSet.cmdChkDir_ACT_END_Addr[MOTOR_Y] = BASEADDR_MOTOR_Y_SYR + (0x30B << 2);
	devSet.cmdChkDir_ACT_END_Addr[MOTOR_Z] = BASEADDR_MOTOR_X_Z + (0x30B << 2);
	devSet.cmdChkDir_ACT_END_Addr[MOTOR_SYR] = BASEADDR_MOTOR_Y_SYR + (0x30F << 2);
	//devSet.cmdChkDir_ACT_END_Addr[MOTOR_VAL] = BASEADDR_MOTOR_X_Z + (0x30F << 2);
	//devSet.cmdChkDir_ACT_END_Addr[MOTOR_INJ] = BASEADDR_MOTOR_X_Z + (0x30F << 2);

	devSet.read_ChkDir_Addr[MOTOR_X] = BASEADDR_MOTOR_X_Z + (0x5 << 2);
	devSet.read_ChkDir_Addr[MOTOR_Y] = BASEADDR_MOTOR_Y_SYR + (0x4 << 2);
	devSet.read_ChkDir_Addr[MOTOR_Z] = BASEADDR_MOTOR_X_Z + (0x4 << 2);
	devSet.read_ChkDir_Addr[MOTOR_SYR] = BASEADDR_MOTOR_Y_SYR + (0x5 << 2);
	//devSet.read_ChkDir_Addr[MOTOR_VAL] = BASEADDR_MOTOR_X_Z + (0x5 << 2);
	//devSet.read_ChkDir_Addr[MOTOR_INJ] = BASEADDR_MOTOR_X_Z + (0x5 << 2);
#endif
...
//		devState.btMotorState[MOTOR_SYR] = *((uint32_t volatile *)(BASEADDR_MOTOR_Y_SYR));
	devState.btMotorState[MOTOR_SYR] = devState.btMotorState[MOTOR_Y];
...
//		devState.btMotorState[MOTOR_INJ] = *((uint32_t volatile *)(BASEADDR_MOTOR_VAL_INJ));
	devState.btMotorState[MOTOR_INJ] = devState.btMotorState[MOTOR_VAL];

#if CHECK_ENCODER_DIRECTION
	unsigned int chk_Dir_Z,chk_Dir_X,chk_Dir_Y,chk_Dir_S;
	static unsigned int chk_Dir_Z1,chk_Dir_X1,chk_Dir_Y1,chk_Dir_S1;

	devState.chk_Dir[MOTOR_Z] = READ_CHK_DIR(MOTOR_Z);
	chk_Dir_Z = (devState.chk_Dir[MOTOR_Z] & 0x3);
	devState.chk_Dir[MOTOR_X] = READ_CHK_DIR(MOTOR_X);
	chk_Dir_X = (devState.chk_Dir[MOTOR_X] & 0x3);	
	devState.chk_Dir[MOTOR_Y] = READ_CHK_DIR(MOTOR_Y);
	chk_Dir_Y = (devState.chk_Dir[MOTOR_Y] & 0x3);
	devState.chk_Dir[MOTOR_SYR] = READ_CHK_DIR(MOTOR_SYR);
	chk_Dir_S = (devState.chk_Dir[MOTOR_SYR] & 0x3);	
#if 1	
	if(chk_Dir_Z != chk_Dir_Z1)
	{
dprintf("[Change Dir] MOTOR_Z --");	
		//if(devState.moving[MOTOR_Z]) 
		{		
			if(chk_Dir_Z == CHK_DIR_ERR) { // || chk_Dir_Z == CHK_DIR_NONE) {
				printf("########MOTOR_Z : DIR CHECK ERROR!!!");
				printf("[Z %08x] [Y %08x] [X %08x] [S %08x]",devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]); 
			}
		}
dprintf("\n");			
	}
	chk_Dir_Z1 = chk_Dir_Z;

	if(chk_Dir_X != chk_Dir_X1) 
	{	
dprintf("[Change Dir] MOTOR_X --");	
		//if(devState.moving[MOTOR_X]) 
		{				
			if(chk_Dir_X == CHK_DIR_ERR) {// || chk_Dir_X == CHK_DIR_NONE) {
				printf("########MOTOR_X : DIR CHECK ERROR!!!");
				printf("[Z %08x] [Y %08x] [X %08x] [S %08x]" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);
			}
		}
dprintf("\n");					
	}
	chk_Dir_X1 = chk_Dir_X;

	if(chk_Dir_Y != chk_Dir_Y1) 
	{
dprintf("[Change Dir] MOTOR_Y --");	
		//if(devState.moving[MOTOR_Y]) 
		{				
			if(chk_Dir_Y == CHK_DIR_ERR) {// || chk_Dir_Y == CHK_DIR_NONE) {
				printf("########MOTOR_Y : DIR CHECK ERROR!!!");
				printf("[Z %08x] [Y %08x] [X %08x] [S %08x]" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);		
			}
		}
dprintf("\n");					
	}
	chk_Dir_Y1 = chk_Dir_Y;

	if(chk_Dir_S != chk_Dir_S1) 
	{	
dprintf("[Change Dir] MOTOR_SYR --");	
		//if(devState.moving[MOTOR_SYR]) 
		{
			if(chk_Dir_S == CHK_DIR_ERR) {// || chk_Dir_S == CHK_DIR_NONE) {
				printf("########MOTOR_SYR : DIR CHECK ERROR!!!");
				printf("[Z %08x] [Y %08x] [X %08x] [S %08x]" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);	
			}	
			else {
	//		dprintf("MOTOR_SYR : [Z %08x] [Y %08x] [X %08x] [S %08x]\n" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);	
			}
		}
dprintf("\n");					
	}
	
	chk_Dir_S1 = chk_Dir_S;	
#endif	
//	devState.chk_Dir[MOTOR_VAL] = READ_CHK_ZX;
//	devState.chk_Dir[MOTOR_INJ] = READ_CHK_YS;	
#endif

#if 0 //CHECK_ENCODER_DIRECTION
	if((devState.chk_Dir[MOTOR_Z] & 0x3) == CHK_DIR_ERR) {
//		errorFunction(SELF_ERROR_CHECK_ENCODER_Z);
		dfp("MOTOR_Z : DIR CHECK ERROR!!!");
dprintf("[Z %08x] [Y %08x] [X %08x] [S %08x]\n",devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]); 
	}

	if((devState.chk_Dir[MOTOR_X] & 0x3) == CHK_DIR_ERR) {
//		errorFunction(SELF_ERROR_CHECK_ENCODER_X);
		dfp("MOTOR_X : DIR CHECK ERROR!!!");
dprintf("[Z %08x] [Y %08x] [X %08x] [S %08x]\n" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);		
	}
	if((devState.chk_Dir[MOTOR_Y] & 0x3) == CHK_DIR_ERR) {
//		errorFunction(SELF_ERROR_CHECK_ENCODER_Y);
		dfp("MOTOR_Y : DIR CHECK ERROR!!!");
dprintf("[Z %08x] [Y %08x] [X %08x] [S %08x]\n" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);		
	}
	if((devState.chk_Dir[MOTOR_SYR] & 0x3) == CHK_DIR_ERR) {
//		errorFunction(SELF_ERROR_CHECK_ENCODER_SYR);
		dfp("MOTOR_SYR : DIR CHECK ERROR!!!");
dprintf("[Z %08x] [Y %08x] [X %08x] [S %08x]\n" ,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);		
	}
#endif
