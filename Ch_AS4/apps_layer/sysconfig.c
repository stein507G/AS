...
//devState.envmDataAvailable = RE_ERR; 
...
//	iprintf("as_INFO.cModel = [%s]\n",as_INFO.cModel);	
//	save_Storage_INFO_Ver_Envm();

	if(isFirmwareUpdate() == RE_ERR || devState.envmDataAvailable == RE_ERR) {
iprintf("save_Storage_INFO_Ver_Envm()\n");
		save_Storage_INFO_Ver_Envm();
	}
...
iprintf("[New MAC] my_mac[0]=[0x%x] [0x%x] [0x%x] [0x%x] [0x%x] [0x%x] \n\r",
	my_mac[0],my_mac[1],my_mac[2],my_mac[3],my_mac[4],my_mac[5]);
			...
			memcpy(as_INFO.cMacAddress, my_mac, sizeof(my_mac));
			NVM_write(INFO_MACADDR_ENVM_ADDR, my_mac, INFO_MACADDR_ENVM_LEN, NVM_DO_NOT_LOCK_PAGE);
#else
			macAddr[0] = 0xAA;
			macAddr[1] = 0xAA;
			macAddr[2] = 0xAA;
...
//===== Envm Data 저장 =====
// Common 영역 0x3F0FC 의 값이 0xAA5555AA가 아니면 실행한다.
// <<Common영역  >> 
//	  Ver 저장 : app Ver, FPGA Ver
// 		Serial No
// 		macAddr
//		model
// <<EEPROM Data>>
...
//	memcpy(&idata[8], oven_INFO.cNetmask, 4);		 - netmask, port no 고정
...
{
//	gOVEN_CONFIG.fMaxTemperature = OVEN_TEMP_SET_MAX; // 최대설정 온도
//	gOVEN_CONFIG.fMinTemperature = OVEN_TEMP_SET_MIN; // 최소설정 온도
...
int save_Storage_INFO_Ver_Envm()
{
	uint32_t volatile addr;
	uint32_t len;
	unsigned int UsedENVM = COPY_DONE_FRAM_TO_ENVM;

// =========================================================================================
	addr = INFO_COMMON_VER_SER_ENVM_ADDR + APP_VER_STORAGE_OFFSET;
	len = 8;
	NVM_write(addr, (const uint8_t *)&as_INFO.FirmwareVersion, len, NVM_DO_NOT_LOCK_PAGE);

// =========================================================================================	
	addr = INFO_COMMON_VER_SER_ENVM_ADDR + MODEL_STORAGE_OFFSET;
	len = 12;
	NVM_write(addr, (const uint8_t *)&as_INFO.cModel, len, NVM_DO_NOT_LOCK_PAGE);

// =========================================================================================
	addr = INFO_COMMON_VER_SER_ENVM_ADDR + USED_ENVM_STORAGE_OFFSET;
	len = 4;
	NVM_write(addr, (const uint8_t *)&UsedENVM, len, NVM_DO_NOT_LOCK_PAGE);
