...
//=====================================================	
	...
	switch(subMenu_sma)	{
	case 1:
		motorSleepCmd(MOTOR_X	,	HOME_DIR_X);			motorSleepCmd(MOTOR_Y	,	HOME_DIR_Y);
		motorSleepCmd(MOTOR_Z	,	HOME_DIR_Z);			motorSleepCmd(MOTOR_SYR	,	HOME_DIR_SYR);
		motorSleepCmd(MOTOR_VAL	,	HOME_DIR_VAL);			motorSleepCmd(MOTOR_INJ	,	HOME_DIR_INJ);
		++subMenu_sma;
		break;
	case 2:
		if(sleepChk(MOTOR_X)			&& sleepChk(MOTOR_Y) 
			&& sleepChk(MOTOR_Z)		&& sleepChk(MOTOR_SYR) 
			&& sleepChkVal(MOTOR_VAL)	&& sleepChkVal(MOTOR_INJ)	) 
		{
			subMenu_sma	=	1;
...
// Z축은 홈다시 찾지 않음
			...
			break;
		case 10:
//dprintf("devState.homeFindEnd[MOTOR_Z]=%d\n",devState.homeFindEnd[MOTOR_Z]  );		
// 실행안한다.
...
//					devState.auxOnOffCnt =
	...
	// LED
#if COMMAND_END_CMD_FOR_ANALYSYS_END
//	LED_Act_Analysis_blink();
	LED_Act_Analysis();
//	iprintf("start LED_Act_Analysis()\n");
...
#if SAMPLE_VOLUME_FIX_PULL_LOOP
							sampleVolume += flushVolume;
#else
...
//=======================================================================
//devState.missVialSkip = YES;
							subMenu_mpi	= 26;//	GO_VIAL_NOT_EXIST_mpi_no;	// 배출후 다음바이알
						}
						else {
							errorFunction(SELF_ERROR_MISSING_VIAL_WAIT);
							subMenu_mpi++;
...
// 제어기 응답을 기다린다.// [skip]  - 다음 바이알 [retry] - 다시 시도  [abort] - ( stop ,cancel , end)  - 현재 시퀀스 분석종료
					switch(devState.missVialResponse) {
						...
						case MISSING_VIAL_SKIP:
							errorFunction(SELF_ERROR_MISSING_VIAL_SKIP);
							devState.curInjCnt = devState.injectionTimes;		// 남은	주입 모두	Cancel - 다음	Sample로														
							subMenu_mpi	= 26;//GO_VIAL_NOT_EXIST_mpi_no;
							break;
						case MISSING_VIAL_RETRY:
//=======================================================================
// Retry
...
void convert_AdjustData_eeprom2envm()
{
	eepromRead_Data(HOME_ADJUST_DATA_STARTADDR,sizeof(devSet.adjust),devSet.adjust.saveFlag); 
	//	NVM_write((ENVM_HOME_ADJUST_DATA_ADDR), &devSet.adjust.saveFlag, sizeof(devSet.adjust), NVM_DO_NOT_LOCK_PAGE);
	NVM_write((ENVM_HOME_ADJUST_DATA_ADDR), &devSet.adjust.saveFlag[0], sizeof(devSet.adjust), NVM_DO_NOT_LOCK_PAGE);
	
	eepromRead_Data(HOME_ADJUST_2ND_STARTADDR,sizeof(devSet.adjust_2nd),devSet.adjust_2nd.saveFlag);	// newVersion = YES;
	NVM_write((ENVM_HOME_ADJUST_DATA_ADDR_2nd), &devSet.adjust_2nd.saveFlag[0], sizeof(devSet.adjust_2nd), NVM_DO_NOT_LOCK_PAGE);
}

void init_AdjustData()
{
	if(devState.envmDataAvailable == RE_ERR) {	// eeprom --> envm
		convert_AdjustData_eeprom2envm();
	}
//epromRead_Data(HOME_ADJUST_DATA_STARTADDR,sizeof(devSet.adjust),devSet.adjust.saveFlag);
	MSS_NVM_read( (uint8_t *)ENVM_HOME_ADJUST_DATA_ADDR, &devSet.adjust.saveFlag, sizeof(devSet.adjust));
	checkAdjustData();
}
...
//	eepromRead_Data(HOME_ADJUST_2ND_STARTADDR,sizeof(devSet.adjust_2nd),devSet.adjust_2nd.saveFlag);	// newVersion = YES;
	MSS_NVM_read((uint8_t *)ENVM_HOME_ADJUST_DATA_ADDR_2nd, &devSet.adjust_2nd.saveFlag, sizeof(devSet.adjust_2nd));
...
//		eepromWrite_Data(HOME_ADJUST_DATA_STARTADDR,sizeof(devSet.adjust) + 1,devSet.adjust.saveFlag);		
		NVM_write((ENVM_HOME_ADJUST_DATA_ADDR), &devSet.adjust.saveFlag[0], (sizeof(devSet.adjust) + 1), NVM_DO_NOT_LOCK_PAGE);		
...
dp("devSet.adjust.rightVial_x[sysConfig.trayRight] = %d\n",devSet.adjust.rightVial_x[sysConfig.trayRight]);
...
int actionInitialize(int value)	// AS_ACTION_INITIALIZE
{
#if	DEBUG_LCASACTION_SUBMENU_NO
	...
	switch(devState.actionSubmenu) {
		...
		case AS_INIT_SUB_FIND_HOME_ALL:
...
#if CHECK_AS_INIT_POSITION_ENCODER_TEST
					devState.actionSubmenu = AS_INIT_POSITION_ENCODER_TEST;
#else
			...
			break;
#if CHECK_AS_INIT_POSITION_ENCODER_TEST
		case AS_INIT_POSITION_ENCODER_TEST:
			if(	AS_INIT_Position_Encoder_Check()) {
				devState.actionSubmenu = AS_INIT_SUB_WASH_SYRINGE;
			}
			break;
#endif			
...
//							devState.mixPrgmFinish = NO;
				...
				case AS_RUN_SUB_WASH_BETWEEN_CHECK_BEFORE:
					if(runSetup.washBetween	== WASH_BETWEEN_NONE)	devState.actionSubmenu = AS_RUN_SUB_GO_VIAL_AND_INJECTION;
						...
						if(devState.doNextSample ==	YES) devState.actionSubmenu	=	AS_RUN_SUB_NEEDLE_WASH_BEFORE;
						else							devState.actionSubmenu	=	AS_RUN_SUB_GO_VIAL_AND_INJECTION;
					}
...
// 소스추가
#if COMMAND_END_CMD_FOR_ANALYSYS_END
dprintf("======XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX=================================LED_Act_Analysis()\n");
#else
#endif
dprintf("WAIT	<<<<<<<<<<<<<>\n <<<analysisTimeCount= %d analysisTime100ms = %d isNextInjection = %d >>>\n",devState.analysisTime100msCnt,devState.analysisTime100ms,devState.isNextInjection);
						}
//===================================================
					break;
...
dprintf("memcpy(&devSet.adjustTmp=%d\n",devSet.adjustTmp.saveFlag[0]);
			}
			break;
...
// convert eeprom data --> envm data 
void convert_SystemInform_eeprom2envm()
{
	uint8_t data[50];
...
iprintf("SYSTEM_INFORM_DATA_SAVE if(saveFlag == DATA_SAVE_FLAG) ");
		memcpy(sysInform.cSerialNo, &data[1], SYSTEM_INFORM_DATA_READ_SERIAL);
		memcpy(as_INFO.cSerialNO, sysInform.cSerialNo, SYSTEM_INFORM_DATA_READ_SERIAL);

		as_INFO.cInstallDate[0] = 20;
		sysInform.cInstallDate.year =	as_INFO.cInstallDate[1] = data[18];
		sysInform.cInstallDate.month = as_INFO.cInstallDate[2] = data[19];
		sysInform.cInstallDate.date =as_INFO.cInstallDate[3] = data[20];
//		memcpy(as_INFO.cInstallDate, data[18], 3);
		...
		NVM_write(INFO_SERIAL_NO_ENVM_ADDR, &as_INFO.cSerialNO[0], INFO_SERIAL_NO_ENVM_LEN, NVM_DO_NOT_LOCK_PAGE);
		NVM_write(INFO_INSTALL_DATE_ENVM_ADDR, &as_INFO.cInstallDate[0], INFO_INSTALL_DATE_ENVM_LEN, NVM_DO_NOT_LOCK_PAGE);
	}
//	else initSystemInform_envm();

// =================== IP ADDR 
	eepromRead_Data(ETHERNET_CONFIG_DATA_SAVE,ETHERNET_CONFIG_DATA_READSIZE,data);
	saveFlag = data[0];

//	if(devSet.IP_switch == 7) { // default ip사용 // DEFAULT_IPADDRESS - 저장
//		init_IpAddr(0);
//		saveIP = YES;
//	
//		save_IPADDR();
...
//	else {
		if(saveFlag == DATA_SAVE_FLAG) {
iprintf("ETHERNET_CONFIG_DATA_SAVE if(saveFlag == DATA_SAVE_FLAG) {");
...
// =================== Serial No + install Date
	MSS_NVM_read( (uint8_t *)INFO_ENVM_SAVE_FLAG_ADDR, saveEnvmFlag, INFO_ENVM_SAVE_FLAG_LEN);
	if(saveEnvmFlag[FLAG_SERIAL_NO] == DATA_SAVE_FLAG) {
		MSS_NVM_read( (uint8_t *)INFO_SERIAL_NO_ENVM_ADDR, data, INFO_SERIAL_NO_ENVM_LEN);
		memcpy(as_INFO.cSerialNO, &data[0], (INFO_SERIAL_NO_ENVM_LEN));
//		memcpy(sysInform.cSerialNo, as_INFO.cSerialNO, SYSTEM_INFORM_DATA_READ_SERIAL);	
printf("####### read Serial No : [%s] [%c]\n", &data[0], data[0]);

		MSS_NVM_read( (uint8_t *)INFO_INSTALL_DATE_ENVM_ADDR, data, INFO_INSTALL_DATE_ENVM_LEN);
		memcpy(as_INFO.cInstallDate, &data[0], INFO_INSTALL_DATE_ENVM_LEN);
//		sysInform.cInstallDate.year = as_INFO.cInstallDate[1];
//		sysInform.cInstallDate.month = as_INFO.cInstallDate[2];
//		sysInform.cInstallDate.date =as_INFO.cInstallDate[3];
	}		
	else {
...
//saveIP = YES;
		...
		if(saveEnvmFlag[FLAG_IP_ADDR] == DATA_SAVE_FLAG) {
			MSS_NVM_read( (uint8_t *)INFO_IP_ADDR_ENVM_ADDR, data, INFO_IP_ADDR_ENVM_LEN);
iprintf(" 	#read IP Config!!! \n");
			memcpy(my_ip, &data[4], 4);
			my_ip[3] = data[4 + 3] + devSet.IP_switch;
...
//	strcpy(sysInform.cIPAddress, "10.10.10.52");
	sprintf(sysInform.cIPAddress,"%d.%d.%d.%d",my_ip[0], my_ip[1], my_ip[2], my_ip[3]);
	as_INFO.PortNo = IP_PORT_NUMBER;
...
//==========================================================================================
	...
	strcpy(ethernetConfig.cPortNo, CIP_PORT_NUMBER);
...
void initSystemInform()
{
	...
	if(devState.envmDataAvailable == RE_ERR)
	{	// eeprom --> envm
		convert_SystemInform_eeprom2envm();
	}

	initSystemInform_envm();
}
...
//printf("saveSystemInform(%d)\n",who);	
}

#if 1
int save_SerialNo_InstallDate_Envm()
{
	uint32_t volatile addr;
	uint32_t len;
	uint8_t idata[INFO_SERIAL_NO_ENVM_LEN]; // = "XXXXCO2020222222";
...
//	memcpy(&idata[0] ,&oven_INFO.cInstallDate[0], sizeof(oven_INFO.cInstallDate));
//	ret = NVM_write(INFO_INSTALL_DATE_ENVM_ADDR, idata, INFO_INSTALL_DATE_ENVM_LEN, NVM_DO_NOT_LOCK_PAGE);
	save_InstallDate_Envm();

//iprintf("#NVM_write ret=%d\n", ret);
...
int save_InstallDate_Envm()
{
	nvm_status_t ret;
//	memcpy(&idata[0] ,&oven_INFO.cInstallDate[0], sizeof(oven_INFO.cInstallDate));
	ret = NVM_write(INFO_INSTALL_DATE_ENVM_ADDR, &as_INFO.cInstallDate[0], INFO_INSTALL_DATE_ENVM_LEN, NVM_DO_NOT_LOCK_PAGE);
	return 0;
}
#endif
...
void convert_Macaddr_eeprom2envm()
{
...
void init_Macaddr_Value()
{
	if(devState.envmDataAvailable == RE_ERR) {	// eeprom --> envm
		convert_Macaddr_eeprom2envm();
	}
//	init_Macaddr_Value_envm();
...
//	LcasState.state = STATE_INITILIZE
	...
	devState.actionMenu	=	AS_ACTION_INITIALIZE;
...
//	devState.adReadValid	=	NO;
	...
	for(int i=0;i<6;i++) devState.moving[i] = 0;
...
int check_SystemConfig_envmdata(uint8_t *data)
{
		SYSTEM_CONFIG_t *tmp;

		tmp = (SYSTEM_CONFIG_t *)data; 
...
// 저장되어 있으면 처리하고 아니면 스킵
void convert_SystemConfig_eeprom2envm()
{
...
//	int dataError;
	...
	eepromRead_Data(SYSTEM_CONFIG_DATA_SAVE,(sizeof(SYSTEM_CONFIG_t) + 1),readSysCfgBuf);
	if(readSysCfgBuf[0] == DATA_SAVE_FLAG) {
//		memcpy(&sysConfig, &readSysCfgBuf[1], size);		
		memcpy(&tmp, &readSysCfgBuf[1], sizeof(SYSTEM_CONFIG_t));

		if(check_SystemConfig_envmdata((uint8_t *)&tmp)) {
//			memcpy(&sysConfig, &readSysCfgBuf[1], size);		
//			dataError = NO;
			
			uint8_t saveFlag = DATA_SAVE_FLAG;
			NVM_write((ENVM_SYSTEM_CONFIG_DATA_ADDR), &saveFlag, 1, NVM_DO_NOT_LOCK_PAGE);
			NVM_write((ENVM_SYSTEM_CONFIG_DATA_ADDR + 1), (uint8_t *)&tmp, ENVM_SYSTEM_CONFIG_DATA_LEN, NVM_DO_NOT_LOCK_PAGE);
		}
		else {
			// 데이터 에러 - 
		}
...
/*
	...
	//			|| tmp.usePrepMode > USE_PREP_MODE_YES
		...
		{ // 초기화 해야 한다.
			...
			uint8_t saveFlag = DATA_SAVE_FLAG;
			NVM_write((ENVM_SYSTEM_CONFIG_DATA_ADDR), &saveFlag, 1, NVM_DO_NOT_LOCK_PAGE);
			ret = NVM_write((ENVM_SYSTEM_CONFIG_DATA_ADDR + 1), tmp, ENVM_SYSTEM_CONFIG_DATA_LEN, NVM_DO_NOT_LOCK_PAGE);
		}
*/		
...
void initSystemConfig_envm()
{
	SYSTEM_CONFIG_t tmp;
	unsigned char saveFlag;// = DATA_SAVE_FLAG;
	int dataError;

	MSS_NVM_read( (uint8_t *)ENVM_SYSTEM_CONFIG_DATA_ADDR, &saveFlag, 1);
	if(saveFlag == DATA_SAVE_FLAG) {
		MSS_NVM_read( (uint8_t *)(ENVM_SYSTEM_CONFIG_DATA_ADDR + 1), &tmp, ENVM_SYSTEM_CONFIG_DATA_LEN);

		if(check_SystemConfig_envmdata((uint8_t *)&tmp)) {			// load 
			dataError = NO;
			memcpy(&sysConfig, &tmp, sizeof(sysConfig));
		}
		else dataError = YES;		// 초기화 
	}
	else dataError = YES;
...
//	unsigned char saveFlag;
	...
	if(devState.envmDataAvailable == RE_ERR) {	// eeprom --> envm
		convert_SystemConfig_eeprom2envm();
	}
	initSystemConfig_envm();
}
...
//		eepromRead_Data(SPECIAL_DATA_SAVE,(size + 1),&readBuf);
		MSS_NVM_read((uint8_t *)INFO_SPECIAL_ENVM_ADDR, &readBuf, (size+ 1));
...
void convert_Specia_eeprom2envm()
{
	uint8_t data[21];

//	size = sizeof(special);
//	eepromRead_Data(TEMP_CAL_DATA_SAVE,(INFO_TEMP_CAL_ENVM_LEN+1),data);
//	eepromRead_Data(SPECIAL_DATA_SAVE,(sizeof(special) + 1),&data);
	eepromRead_Data(SPECIAL_DATA_SAVE,(sizeof(special) + 1),data);

	if(data[0] == DATA_SAVE_FLAG) {
		NVM_write((INFO_SPECIAL_ENVM_ADDR), data, (sizeof(special) + 1), NVM_DO_NOT_LOCK_PAGE);	
		dp("convert_Specia_eeprom2envm() \n");
	}
}

void init_SpecialData()
{
	if(devState.envmDataAvailable == RE_ERR) {	// eeprom --> envm
		convert_Specia_eeprom2envm();
	}
...
int usedTimeCheck(unsigned char act)
{
	...
	switch(act) {
		case DATA_LOAD:
			MSS_NVM_read((uint8_t *)ENVM_USED_TIME_DATA_ADDR, &saveFlag,   1);
		
			if(saveFlag == DATA_SAVE_FLAG) {
//				MSS_NVM_read((uint8_t *)ENVM_USED_TIME_DATA_ADDR, (uint8_t *)&usedTime,	 sizeof(usedTime));
				MSS_NVM_read((uint8_t *)(ENVM_USED_TIME_DATA_ADDR+1), (uint8_t *)&usedTime,	 sizeof(usedTime));

				ret = OK;
iprintf("usedTimeCheck\n");				
			}
			else {
...
//				usedTime.needle = 0;	// 이동거리 cm
		...
		case DATA_SAVE:
			if(LcasState.state == STATE_READY) {
				...
				NVM_write((ENVM_USED_TIME_DATA_ADDR), data, (sizeof(usedTime) + 1), NVM_DO_NOT_LOCK_PAGE);
...
void convert_usedTime_eeprom2envm()
{
	uint8_t data[ENVM_USED_TIME_DATA_LEN + 1];

	eepromRead_Data(USED_TIME_DATA_SAVE, sizeof(usedTime) + 1 , data);
	if(data[0] == DATA_SAVE_FLAG) {
//		NVM_write((ENVM_USED_TIME_DATA_ADDR), &data, (sizeof(usedTime) + 1), NVM_DO_NOT_LOCK_PAGE);		
		NVM_write((ENVM_USED_TIME_DATA_ADDR), data, (sizeof(usedTime) + 1), NVM_DO_NOT_LOCK_PAGE); 	
