...
// 20210826
// Full loop : Sampling loop 200u 오류 : 너무많은 양을 sampling한다.
//		#define FIRMWARE_VERSION		" 5.13"	// 맨앞은 space
//		#define MACHINE_VER		" 5.13"

// 20210910
	// RTC disp 오류
//		#define FIRMWARE_VERSION		" 5.14"	// 맨앞은 space
//		#define MACHINE_VER		" 5.14"

// 20211222	-- Leak Error 수정 : Stop 이나 Init 명령이후 동작안함.
//#define AS_COOLING_OPTION			1
//		#define FIRMWARE_VERSION		" 5.15"	// 맨앞은 space
//		#define MACHINE_VER		" 5.15"

// 20220308
// #define CHROZEN_Z_HEIGHT_SENSOR_MODIFY		1
//		#define FIRMWARE_VERSION		" 5.16" // 맨앞은 space
...
// MIX_  table - 10개로 - RS422_ENABLE
// 펌웨어 구분은 FIRMWARE_VERSION으로 구분한다.
	//#define	RS422_MIX_STRUCT_DOWNSIZE
	//#define MIX_TABLE_COUNT 	10
// 2021.11.1x - RS422을 위한 조치
//		#define FIRMWARE_VERSION		" 5.20" // 맨앞은 space
//		#define MACHINE_VER 	" 5.20"
//		#define AS_FIRMWARE_VERSION 	520 	

// 20220308
// #define CHROZEN_Z_HEIGHT_SENSOR_MODIFY		1
//		#define FIRMWARE_VERSION		" 5.21" // 맨앞은 space
//		#define MACHINE_VER 	" 5.21"
//		#define AS_FIRMWARE_VERSION 	521 	
...
//	#define VIAL_XY_CALIBRATION_FIX				1
		#define FIRMWARE_VERSION		" 5.22" // 맨앞은 space
		#define MACHINE_VER 	" 5.22"
		#define AS_FIRMWARE_VERSION 	522

// 20220826 
// FPGAVer 11110504
#define FIRMWARE_VERSION		" 5.23" // 맨앞은 space
#define MACHINE_VER 	" 5.23"
#define AS_FIRMWARE_VERSION 	523 	

// 20220831
// FPGAVer 11110505
#define FIRMWARE_VERSION		" 5.24" // 맨앞은 space
#define MACHINE_VER 	" 5.24"
#define AS_FIRMWARE_VERSION 	524
...
// moveStepSpeed 명령에서 입력된 것이 적용된다. -- 아래 CMD_CHK_DIR_STATE_DIR_Z은 사용하지 않음.
// Syringe Motor는 엔코더가 반대로 장착되어 있다. 출력되는 값이 반대임. - FPGA에서 옵션사항으로 추가 함. 
...
#else
	#if CHECK_DIR_MOVING_STATE
		#define DEVSTATE_MOVING(WHO,X)	devState.moving[WHO] = X;
	#else
		#define DEVSTATE_MOVING(WHO,X)
	#endif
...
// READ_CHK_DIR_Z - bit[1:0]-DIr / bit[5:2]-Pos / bit[7]Break / bit[6] Error
	#define READ_CHK_DIR_Z		*((uint32_t volatile *)(devSet.baseAddr[MOTOR_Z] + (0x4 << 2))) 	// 하위 [15:0]
// READ_CHK_DIR_X - bit[17:16]-DIr / bit[21:18]-Pos / bit[23]Break / bit[22] Error
	#define READ_CHK_DIR_X		*((uint32_t volatile *)(devSet.baseAddr[MOTOR_X] + (0x5 << 2)))		// 상위 [31:16]
	#define READ_CHK_DIR_Y		*((uint32_t volatile *)(devSet.baseAddr[MOTOR_Y] + (0x4 << 2))) 	// 하위 [15:0]
	#define READ_CHK_DIR_SYR	*((uint32_t volatile *)(devSet.baseAddr[MOTOR_SYR] + (0x5 << 2)))		// 상위 [31:16]
#else
...
#endif
	#define READ_CHK_ZX		*((uint32_t volatile *)(devSet.baseAddr[MOTOR_Z] + (0x2 << 2)))		// 하위 [15:0]
	#define READ_CHK_YS 	*((uint32_t volatile *)(devSet.baseAddr[MOTOR_Y] + (0x2 << 2))) 	// 하위 [15:0]
...
//#define READ_CHK_DIR(WHO)		*((uint32_t volatile *)(devSet.baseAddr[WHO] + (0x4 << 2) + (((WHO) & 0x1) << 2))		// 상위 [31:16]
...
#define HOME_FINE_CHECK_MAX_TIME_SYR		7000	// 7초 홈 찾기 
#define HOME_FINE_CHECK_MAX_TIME_VAL		7000	// 7초 홈 찾기 
#define HOME_FINE_CHECK_MAX_TIME_INJ		7000	// 7초 홈 찾기 
...
#define HOME_FINE_CHECK_MAX_TIME_SYR	70000	// 7초 홈 찾기 
#define HOME_FINE_CHECK_MAX_TIME_VAL	70000	// 7초 홈 찾기 
#define HOME_FINE_CHECK_MAX_TIME_INJ	70000	// 7초 홈 찾기 
