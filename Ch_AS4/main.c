...
#if RS422_ENABLE
	devState.rs422if_connected = NO;
	devState.rs422if_packet = NO;
	devState.rs422if_request = NO;
#if RS422_REQUEST_CMD_INDEX_ADD
	devState.rs422if_cmd_index = 0;
#endif
...
//	pLed_Control_Fn = led_Control;
	...
	pAction_Fn[AS_ACTION_INITIALIZE] = actionInitialize;	// AS_ACTION_INITIALIZE
...
// (rxv_intr_ext) RS422수신 인터럽트 발생		 -   My_ID 이거나 Global_ID
			...
			switch(++timeCnt100Hz) {
...
#if RS422_ENABLE
					if(devState.rs422if_connected) break;
#endif
#if LWIP_VERSION_2
				...
				case CNT_EVENT_TIME_BASE:
					if(++timeCnt10Hz == 10)	{// 20220420
						timeCnt10Hz = 0;
						devState.analysisTime100msCnt++;
#if 0//CHECK_ENCODER_DIRECTION
static unsigned int chkDir_cnt = 0;
static unsigned int chkDirZ = 0;
static unsigned int chkDirY = 0;
static unsigned int chkDirX = 0;
static unsigned int chkDirS = 0;

if(devState.chk_Dir[MOTOR_Z] != chkDirZ || devState.chk_Dir[MOTOR_Y] != chkDirY || devState.chk_Dir[MOTOR_X] != chkDirX || devState.chk_Dir[MOTOR_SYR] != chkDirS) 
					dprintf("[%d]	[Z %08x] [Y %08x] [X %08x] [S %08x]\n",++chkDir_cnt,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);
chkDirZ = devState.chk_Dir[MOTOR_Z];
chkDirY = devState.chk_Dir[MOTOR_Y];
chkDirX = devState.chk_Dir[MOTOR_X];
chkDirS = devState.chk_Dir[MOTOR_SYR];
#endif
					}					
					pTime_Event_Fn();
					pLed_Control_Fn();
...
static unsigned char subMenu = 0;

if(menu != devState.actionMenu || subMenu != devState.actionSubmenu) {
	menu = devState.actionMenu;
	subMenu = devState.actionSubmenu;
	dprintf("##[ ACTION: Menu[%d] Submenu[%d] ]##\n",
		devState.actionMenu,
	...
	static unsigned int chkDirZ1 = 0;
	static unsigned int chkDirY1 = 0;
	static unsigned int chkDirX1 = 0;
	static unsigned int chkDirS1 = 0;


	chkDirZ1 = devState.chk_Dir[MOTOR_Z] & 0x03;
	chkDirY1 = devState.chk_Dir[MOTOR_Y] & 0x03;;
	chkDirX1 = devState.chk_Dir[MOTOR_X] & 0x03;;
	chkDirS1 = devState.chk_Dir[MOTOR_SYR] & 0x03;;
	
//	if(devState.chk_Dir[MOTOR_Z] != chkDirZ || devState.chk_Dir[MOTOR_Y] != chkDirY || devState.chk_Dir[MOTOR_X] != chkDirX || devState.chk_Dir[MOTOR_SYR] != chkDirS) 
	if(chkDirZ1 != chkDirZ || chkDirY1 != chkDirY || chkDirX1 != chkDirX || chkDirS1 != chkDirS) 		
						dprintf("[%d]	[Z %08x] [Y %08x] [X %08x] [S %08x]\n",++chkDir_cnt,devState.chk_Dir[MOTOR_Z],devState.chk_Dir[MOTOR_Y],devState.chk_Dir[MOTOR_X],devState.chk_Dir[MOTOR_SYR]);
	chkDirZ = chkDirZ1;//devState.chk_Dir[MOTOR_Z];
	chkDirY = chkDirY1;//devState.chk_Dir[MOTOR_Y];
	chkDirX = chkDirX1;//devState.chk_Dir[MOTOR_X];
	chkDirS = chkDirS1;//devState.chk_Dir[MOTOR_SYR];
#endif
